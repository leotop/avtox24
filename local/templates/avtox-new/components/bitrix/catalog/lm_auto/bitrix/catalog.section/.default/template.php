...
<table cellpadding="0" cellspacing="0" border="0">
		...
		<td valign="top"  width="<?=round(100/$arParams["LINE_ELEMENT_COUNT"])?>%" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
			...
			<table class="item" data="location.href='<?=$arElement["DETAIL_PAGE_URL"]?>'" cellpadding="0" cellspacing="2" border="0">
				<?if(is_array($arElement["PREVIEW_PICTURE"])):?>
					<td valign="center" height="230">
					<? $image_resize = CFile::ResizeImageGet($arElement["PREVIEW_PICTURE"], array("width" => 200, "height" => 200));?>
					<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="<?=$image_resize["src"]?>" width="<?=$arElement["PREVIEW_PICTURE"]["WIDTH"]?>" height="<?=$arElement["PREVIEW_PICTURE"]["HEIGHT"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a><br />
					</td>
				</tr>
				<tr>
				<?elseif(is_array($arElement["DETAIL_PICTURE"])):?>
					<td valign="center" height="230">
					<? $image_resize = CFile::ResizeImageGet($arElement["DETAIL_PICTURE"], array("width" => 200, "height" => 200));?>
					<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img border="0" src="<?=$image_resize["src"]?>" width="<?=$arElement["DETAIL_PICTURE"]["WIDTH"]?>" height="<?=$arElement["DETAIL_PICTURE"]["HEIGHT"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a><br />
					</td>
				</tr>
				<tr>
				<?endif?>
				<tr>
					<td valign="middle"><p class="name"><a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement["NAME"]?></a></p>
						<?foreach($arElement["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
							<? if($pid == "action"){ ?>
								<?=$arProperty["NAME"]?>:&nbsp;
								<? foreach($arProperty['VALUE'] AS $id_action){ ?>
									<? echo "<a href=\"" . $arResult['DETAIL_PAGE_URL_ACTION'][$id_action] . "\"><img class=\"action\" src=\"" . $arResult['PREVIEW_PICTURE_ACTION'][$id_action] . "\"></a>"; ?>
								<? } ?>
							<? }else{ ?>
								<?=$arProperty["NAME"]?>:&nbsp;
							...
							<?foreach($arElement["OFFERS"] as $arOffer):?>
								...
								<?if($arOffer["CAN_BUY"]):?>
									...
									<?else:?>
										...
										&nbsp;<a class="btn btn-mini" href="<?echo $arOffer["ADD_URL"]?>" rel="nofollow"><?echo GetMessage("CATALOG_ADD")?></a>
										</noindex>
									<?endif;?>
							...
							<?if($arElement["CAN_BUY"]):?>
                                ...
                                <div class="buy_it">
                                    ...
                                    */
                                    ...
                                    $url = $arElement["DETAIL_PAGE_URL"]. '?ajax=1';
                                    ...
                                    ?>       
                                    <div class="btn-group">
                                    <a href="<?=$arElement["DETAIL_PAGE_URL"]?>" class="btn btn-info btn-mini">Подробно</a>
                                    
                                    <a href="<?=$arElement["DETAIL_PAGE_URL"]?>" onclick="<?=$href?>;return false;" class=" btn-mini btn express_show btn-warning">Экспресс просмотр</a>                         
                                    </div>
                                    
                                    
                                    <!--a class="btn btn-warning" href="<?echo $arElement["BUY_URL"]?>" rel="nofollow"><?echo GetMessage("CATALOG_BUY")?></a>&nbsp;
                                    <a class="btn btn-mini" href="<?echo $arElement["ADD_URL"]?>" rel="nofollow"><?echo GetMessage("CATALOG_ADD")?></a-->
									</noindex>
                                </div>
								<?endif;?>
