...
$contemporaryYear = !empty($arParams['CONTEMPORARY_YEAR']) ? intval($arParams['CONTEMPORARY_YEAR']) : 2000;

if ($contemporaryYear != null) {

	$filePath = __DIR__.'/contemporary_year.txt';
	$oldContemporaryYear = file_get_contents($filePath);

	if (!file_exists($filePath) || $contemporaryYear != $oldContemporaryYear) {
		file_put_contents($filePath, $contemporaryYear);
		setcookie('contemporary_year', $contemporaryYear, time() + 3600);
	}
	else {
		setcookie('contemporary_year', $oldContemporaryYear, time() + 3600);
	}
}
else {
	setcookie('contemporary_year', 'allyears', time() + 3600);
}
...
//if ($this->StartResultCache(false, $sCacheID)) {
     ...
     */
    if ($arVariables['BRAND'] == '' && $available_tecdoc == 1) {
        $template = 'brands';
     ...
     */
        ...
        if ($manual_tecdoc_groups != null) {
        	...
        	foreach ($manual_tecdoc_groups as $item) {
        		...
        		$args = array(
        				'brand_id' => $arVariables['BRAND'], 'model_id' => $arVariables['MODEL'], 'type_id' => $arVariables['CAR_ID'], 'group_id' => (int) trim($item),
        				'include_oem' => ($arParams['SHOW_ORIGINAL_ITEMS'] === 'Y'), 'include_info' => ($arParams['INCLUDE_PARTS_IMAGES'] == 'Y')
        		);

        		try {
        			$response_parts = $api->query('getDetails2', $args);
        		} catch (Exception $e) {
        			$arResult['ERROR'] = $e->GetMessage();
        			$this->IncludeComponentTemplate('error');
        			return;
        	...
        	foreach ($arResult['DETAILS'] as $key => $detail) {
        		...
        		*/
        		...
        		if (!empty($arVariables['BRAND'])) {
        			...
        			$arResult['DETAILS'][$key]['search_url'] .= '&tecdoc_folder=' . $arParams['SEF_FOLDER'];
...
// var_dump($arResult['DETAILS']);
    ...
    if (!$USER->IsAuthorized() && isset($_REQUEST['SHOW_AUTH_FORM'])) {
    	...
    	if ($available_tecdoc != 1)
    		$this->IncludeComponentTemplate('unavailable_tecdoc');
    	else
