...
foreach ($arResult['PARTS'] as $group_name => $parts) {
	...
	$parts_collection = array();
	$resorted = array();
	foreach ($parts as $part) {
		$parts_group_id = $part['supplier_id'] . "|" . $part['article'] . "|" . $part['brand_title'];
		if (!is_array($parts_collection[$parts_group_id])) {
			$parts_collection[$parts_group_id] = array();
		}
		array_push($parts_collection[$parts_group_id], $part);
	}

	foreach ($parts_collection as $supplier => $data) {
		uasort($data, "customTableSort");
		array_push($resorted, array_shift($data));
		if (count($data)) {
			uasort($data, "customTableSort");
			array_push($resorted, array_shift($data));
			if (count($data)) {
				$data = array_map(function($v){ $v['hidden_row'] = true; return $v; }, $data);
				$resorted = array_merge($resorted, $data);	
			}
		}
	}

    printPartsTable($resorted, $group_id);
}
...
<table class="lm-auto-search-parts tablesorter silver-table" data-analog-type="<?= $group_id ?>">
    ...
    <? foreach ($parts as $index => $part) { ?>
        ...
        <tr class="hproduct <? if ($part['hidden_row']) { echo "hidden_row"; } else { echo "visible_row"; } ?>" style="<?= $part['supplier']['PROPS']['css']['VALUE'] ?>">
            <? if (!in_array('brand', $GLOBALS['LM_AUTO_SEARCH_RESULTS_HIDE_FIELDS'])) { ?>
        ...
        <? 
        	...
        	if (($part['hidden_row'] && $next_part && !$next_part['hidden_row']) || ($part['hidden_row'] && $last_part == $part)) { ?> 
        	<tr><td colspan="12" class="show_more_products"><?= GetMessage('SHOW_ALL_RESULTS') ?></td></tr> 
        <? } ?>
    <? } ?>
    </tbody>
</table>
