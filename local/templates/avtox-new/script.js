...
$(function() {
    ...
    $(".dropdown-main-menu-button").on("click", function(){
        $(this).parent().toggleClass("active");
    })

    //переключение вкладок для поиска
    $(".main-search-tabs-titles ul li").on("click", function(){      
        var tab_index = $(this).data("search-tab-index");
        //если существует блок, который нужно отобразить
        if ($(".search-tab-content[data-search-tab=" + tab_index + "]").length > 0) {
            $(".main-search-tabs-titles ul li").removeClass("active");
            $(this).addClass("active");
            $(".search-tab-content").removeClass("active");
            $(".search-tab-content[data-search-tab=" + tab_index + "]").addClass("active");
        }
	...
	$(".search_form form").on("submit", function(e){
		...
		var search_value = "",
			search_inputs_values = [];
		
		$(this).find("input[type='text']").each(function(){
			search_inputs_values.push($(this).val());
		});
		
		search_value = search_inputs_values.join("-");
	
		if (!$(".search_block").data("autocomplited") && search_value.length != 0) {
			$.post("/ajax/add_search_tip.php", {
				query: search_value,
				type: $(this).find("input[type='text']").attr("name")
			}, function(result) {
		    });
	    }
	...
	$("#search_tips li").click(function(){
		var search_input_name = $(this).data("input"),
			search_input_value = $(this).data("value"),
			search_input_tag = $("input[name='" + search_input_name + "']");
		
		if (search_input_name.search("frame") == 0) { // есть слово frame, необходимо для двойных инпутов
			var splitted_values = search_input_value.split("-");
			search_input_tag.val(splitted_values[0]);
			search_input_tag.nextAll('input').first().val(splitted_values[1]);
		} else {
			search_input_tag.val(search_input_value);
		}
		$(".search_block").data("autocomplited", "1"); // флаг, означающий, что было автодополнение
		search_input_tag.closest("form").submit();
	});
	
	$(".search_form input").on("focus", function(){
		var input_name = $(this).attr("name");
			
		if (input_name.search("frame") == 0) { // есть слово frame, необходимо для двойных инпутов
			input_name = "frame";
		}
		$("ul[data-search-container='" + input_name + "']").fadeIn(200);
	});
	$(".search_form input").on("blur", function(){
		...
		$("ul[data-search-container='" + input_name + "']").fadeOut(100);
	})
})