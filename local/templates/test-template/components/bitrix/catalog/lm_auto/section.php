...
<?if (CModule::IncludeModule("iblock") && COption::GetOptionString("eshop", "catalogSmartFilter", "Y", SITE_ID)=="Y"){
    ...
    <?$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "new_structur_filter", Array(
	"IBLOCK_TYPE" => "catalog",	// Тип инфоблока
		...
		"SECTION_TITLE" => "UF_BROWSER_TITLE",	// Заголовок
		"SECTION_DESCRIPTION" => "UF_BROWSER_TITLE",	// Описание
		"TEMPLATE_THEME" => "blue",
...
?>
<div class="sort">
	<div class="sorttext"><?=GetMessage('SECT_SORT_LABEL')?>:
    ...
    <?foreach ($arAvailableSort as $key => $val):
        ...
        $icon_sort = ($sort_order == 'asc') ? 'up' : 'down';
        ?>
        <a href="<?=$APPLICATION->GetCurPageParam('sort='.$key.'&order='.$newSort, 	array('sort', 'order'))?>" class="btn btn-mini sortbutton<?=$className?>" rel="nofollow"><?=GetMessage('SECT_SORT_'.$key)?><?if ($sort == $val[0]):?>&nbsp;&nbsp;<i class="icon-chevron-<?=$icon_sort?>"></i><?endif?></a>
    <?endforeach;?>
    </div>
</div>
...
<?$APPLICATION->IncludeComponent(
	...
	$current_view == "bar" ? "" : (($current_view=="price_list") ? "price_list" : ""),
	Array(
		"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
		"IBLOCK_ID" => $arParams["IBLOCK_ID"],
