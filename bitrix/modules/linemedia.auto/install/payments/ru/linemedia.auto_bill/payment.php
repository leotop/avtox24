...
<body bgColor="#ffffff">
<p><b>ПОСТАВЩИК:</b>
<?=$useBranchAccount ? $branchProps['company_name']['VALUE'] : (CSalePaySystemAction::GetParamValue("SELLER_NAME")) ?><br>
Адрес: <?=$useBranchAccount ? $branchProps['address']['VALUE']['TEXT'] : (CSalePaySystemAction::GetParamValue("SELLER_ADDRESS")) ?><br>
Телефон: <?=$useBranchAccount ? $branchProps['phones']['VALUE']['0'] : (CSalePaySystemAction::GetParamValue("SELLER_PHONE"))  ?><br>
ИНН: <?=$useBranchAccount ? $branchProps['INN']['VALUE'] : (CSalePaySystemAction::GetParamValue("SELLER_INN")) ?> / КПП: <?=$useBranchAccount ?  $branchProps['KPP']['VALUE'] :	(CSalePaySystemAction::GetParamValue("SELLER_KPP"))
		?><br>
Банковские реквизиты:<br>
р/с <?=$useBranchAccount ? $branchProps['account']['VALUE'] : (CSalePaySystemAction::GetParamValue("SELLER_RS"))?><br>
к/с <?=$useBranchAccount ? $branchProps['corr_account']['VALUE'] : (CSalePaySystemAction::GetParamValue("SELLER_KS")) ?><br>
БИК <?=$useBranchAccount ? $branchProps['BIK']['VALUE'] : (CSalePaySystemAction::GetParamValue("SELLER_BIK")) ?>
</p>
		...
		{?>
			<tr>
				<td align="right" bgcolor="#ffffff" colspan="4" style="border: 1pt solid #000000; border-right:none; border-top:none;">
					 ...
					 * Кто: Назарков Илья
					 * Задача: 5045
					 * Пояснения: считаем, что влючено ндс в доставку
					 */


					if ($arTaxList['CODE'] == 'NDS') {
						$arTaxList['VALUE_MONEY'] += $arTaxList['VALUE'] / (100 + $arTaxList['VALUE']) * $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE_DELIVERY"];
					}
...
<p><b>Итого к оплате:</b>
	 ...
	 * Кто: Назарков Илья
	 * Задача: 5045
	 * Пояснения: Формируем сумму для суммы словами также, как и сумма в "итого"
	 */
	...
	$numWord = number_format($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"], $arCurFormat["DECIMALS"], $arCurFormat["DEC_POINT"], $arCurFormat["THOUSANDS_SEP"]);
	//$numWord = SaleFormatCurrency($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"], $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"], true);
