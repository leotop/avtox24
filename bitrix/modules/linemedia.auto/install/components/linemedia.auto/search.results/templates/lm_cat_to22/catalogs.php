	...
	$(document).ready(function(){
		...
		$('.lm-auto-search-catalogs-go a').on("click", function() {
         ...
         var new_url = it_url+"&r=ok";
			// var thisBrand = "<strong class=\"thisBrand\"><span class=\"deletBrand\">x</span>бренд <span>"+brand+"</span></strong>";
			$(".modal-body").html('');
			$('.popUp-overlay').hide();
			$('.modal-popUp').hide();
			$('.ItemList .button .CArt<?=$_POST["article"];?>').attr("data-brand",brand).attr('new_url',new_url).delay(2000).trigger('click');
			// $('.ItemList .button .CArt<?=$_POST["article"];?>').parents('.ItemList .button').append(thisBrand);
		});
	});
</script>