				 ...
				 <td class="basket">
					...
					?>			 
					  <a class="btn" href="javascript:void(0);" data-url="<?=$part["buy_url"];?>" data-name="<?=$part["title"];?>" data-brand="<?=$part["brand_title"];?>" data-sku="<?=$part["article"];?>" data-part_id="<?=$curId;?>" data-supplier_id="<?=$part["supplier_id"];?>" data-supplier="<?=$part['supplier']['PROPS']['visual_title']['VALUE'];?>" data-delivery="<?=$part['delivery'];?>" data-price="<?=$part["price"];?>" data-price_src="<?=$part["price_src"];?>" data-chain_id="<?=$part["chain_id"];?>" data-kratnost="<? if($part["multiplication_factor"]>0){?><?=number_format($part["multiplication_factor"], 0, '.', '');?><? }else{ ?>1<? } ?>" data-maxvalue="<?=$part["quantity"];?>" data-hash="<?= $hash ?>" <? if($part['multiplication_factor'] > 1) { ?>data-step="<?=$part['multiplication_factor']?>" data-toggle="tooltip" title="<?=GetMessage('LM_AUTO_SEARCH_MULTIPLICATION_FACTOR', array('N'=>$part['multiplication_factor']))?>"
                        <? } ?>><?=GetMessage('LM_AUTO_CHOOSE_IT')?></a>
				 </td>
            <? // Возврат товара
            if(LinemediaAutoReturnGoods::isStatusEnabled() && !in_array('return', $GLOBALS['LM_AUTO_SEARCH_RESULTS_HIDE_FIELDS'])) { ?>
...
<script>
	...
	$(document).ready(function(){
        ...
        $(".hproduct .btn").on('click', function(){
            ...
            var part_id = getParameterByName("part_id", buy_url);//$(this).data("part_id");
            var extra_id = getParameterByName("extra%5Bid%5D", buy_url);
            ...
            if(maxvalue) {
                 maxvalue = maxvalue.toString().replace(',','').replace('.','');
            }

            var step = $(this).data("step");

            var name = $("#art_<?=$article?>_<?=$key_id;?>").find(".TypeName").html();

            if(maxvalue>1){
                var toggle = $(this).data("toggle");
                var title = "<?=GetMessage('LM_AUTO_CHOOSE_IT')?>"+kratnostVal;
            }
            $(".close-popUp").trigger('click');

            var count = $(this).parents(".hproduct").find(".maxvalue").val();

            $("#art_<?=$article?>_<?=$key_id;?>").addClass("active");
            $("#art_<?=$article?>_<?=$key_id;?>").addClass("green");
            $("#art_<?=$article?>_<?=$key_id;?> .BuyTD a.analog").hide();
            $("#art_<?=$article?>_<?=$key_id;?> .BuyTD").attr("colspan", "2");
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartName").text(name);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartBrand").text(brand);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartSKU").text(sku);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartPrice").text(price);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartSupplier").text(supplier);

            if(kratnost>1){
                $("#art_<?=$article?>_<?=$key_id;?>").find(".PartKratnost").text(kratnost);
            }
            $("#art_<?=$article?>_<?=$key_id;?>").find(".PartBuy").attr("count",count).attr("hash",hash).attr("kratnost",kratnostVal).attr("part_id",
                part_id).attr("price_src",price_src).attr("chain_id",chain_id).attr("max_available_quantity",maxvalue).attr("q","<?=$_REQUEST["article"];?>").attr("onclick",
                    'javascript: add2cart(\"'+ hash +'\",\"'+ buy_url +'\", \"'+ maxvalue +'\");');
            $("#art_<?=$article?>_<?=$key_id;?>").find(".NeedQuanty span").hide();
            $("#art_<?=$article?>_<?=$key_id;?>").find(".IQuanty").show().attr("count",count).attr("part_id",part_id).attr("price_src",price_src).attr("chain_id",chain_id).attr("q","<?=$_REQUEST["article"];?>").attr("brand",brand).attr("name",'quantity[' + hash + ']').attr("data-part-hash",hash).attr("maxValue",maxvalue).attr("data-kratnost", kratnostVal).attr("data-toggle","tooltip").attr("title",title).val(kratnostVal);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".id_inp").attr("name",'part_id[' + hash + ']').val(part_id);

            $("#art_<?=$article?>_<?=$key_id;?>").find(".extra_id").attr("name",'extra[id][' + hash + ']').val(extra_id);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".q_inp").attr("name",'q[' + hash + ']').val(sku);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".supplier_id_inp").attr("name",'supplier_id[' + hash + ']').val(supplier_id);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".brand_title_inp").attr("name",'brand_title[' + hash + ']').val(brand);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".price").attr("name",'price[' + hash + ']').val(price_src);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".price_src").attr("name",'price_src[' + hash + ']').val(price_src);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".max_available_quantity").attr("name",'max_available_quantity[' + hash + ']').val(maxvalue);
            $("#art_<?=$article?>_<?=$key_id;?>").find(".SearchIt").attr("url",'<?= dirname($templateFile).'/analogs.php' ?>');
