...
if(isset($arParams['SEO_BLOCK']) && $arParams['SEO_BLOCK'] == 'Y') {
    ...
    /*
     * Если у нас найден только 1 каталог, то в качестве бренда берем бренд первой оригинальной запчасти
     */
...
/*
 * Изменяем ссылки на каталоги для удаленного поствщика etsp
 */
if(is_array($arResult['CATALOGS'])) {
    foreach ($arResult['CATALOGS'] as $id => $catalog) {
        if(is_array($catalog['source'])) {
            $catalog['source'] = implode(',', $catalog['source']);
        }
