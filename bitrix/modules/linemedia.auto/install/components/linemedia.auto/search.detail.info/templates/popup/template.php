...
<? if ($arParams['SHOW_APPLICABILITY'] == 'Y') { ?>
    ...
    <? if (!empty($arResult['applinceBrandList'])) { ?>
        <div class="applicability" id="lmTemplatePopup" data-article="<?= $arParams['ARTICLE'] ?>" data-brand="<?=$arParams['BRAND']?>">
            <div class="applicability-firms">
                <? foreach ($arResult['applinceBrandList'] as $t) { ?>
                    <a href="javascript:void(0)" class="applicability-firm" data-manuid="<?//= $firm['manuId'] ?>"><?= $t ?></a>
                <? } ?>
            </div>
