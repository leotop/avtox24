$(document).ready(function() {
    ...
    /**
     *	Т.к. ссылка ведет на другую страницу и не отправляется ajax-запрос, чтобы отследить когда можно отключить
     *	отображение элемента с классом lm-page-loading - на данной странице мы его отключаем всегда
     */

    $('.price > a').on('click', function() {
        if($('.lm-page-loading').css('display') == 'block')
    ...
    function onClickSave() {
        ...
        var newTitle = wordwrap(title, string_symbols, "<br/>", true );
         ...
         * отображения, а также запоминаются отредактированные значения
         */
        tr.find(".title-old").text(wordwrap($(this).closest("tr").find(".title-new").val(), string_symbols, " ", true ));
