...
/**
*	Событие на изменение кодировки у $_REQUEST
*/

$events = GetModuleEvents("linemedia.auto", "OnBeforeNotepadAction");
...
if ( $_POST['notepad'] == 'add' && !empty($_POST['part_id']) ) {
    ...
    $arFields = array(
        ...
        'extra'       => strip_tags(trim($_REQUEST['extra']))
    );
...
if ( empty($errors) && $_POST['notepad'] == 'add' || $_POST['notepad'] == 'update' ) {
	...
	/**
	*	Событие на изменение кодировки у $js_send для корректного отображения цены
	*/
	
	$events = GetModuleEvents("linemedia.auto", "OnBeforePricesSend");
	while ($arEvent = $events->Fetch()) {
		try {
			ExecuteModuleEventEx($arEvent, array(&$js_send));
		} catch (Exception $e) {
			throw $e;
		}
	}
