...
$arParams['STRING_SYMBOLS'] = $arParams['STRING_SYMBOLS'] ?: 30;
...
foreach ($arResult['DETAILS'] as $key => $detail) {
    ...
    $findurl = LinemediaAutoUrlHelper::getPartUrl(
        ...
        'extra' => (array)json_decode($detail['extra'])
    ...
    */
    $parts = (array) $search->searchLocalDatabaseForPart(
        ...
        'article' => $detail['article'],
        'brand_title' => $detail['brand_title'],
        'extra' => (array)json_decode($detail['extra']) //в классе реально это не учитывается
        ),
        true
    );
