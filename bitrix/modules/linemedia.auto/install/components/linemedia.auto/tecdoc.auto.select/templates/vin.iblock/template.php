...
<script type="text/javascript">
	...
	var MODIFICATIONS_SET = '<?=$arParams['MODIFICATIONS_SET']?>';
</script>
<? if (is_array($arResult['brands']) && count($arResult['brands']) > 0) { ?>
    <tr id="tr_brand">
        <td class="field-name" valign="top"><?= GetMessage('MARK') ?>:</td>
        <td id="td_brand">
            ...
            <? if ($arParams['BRAND_ID'] > 0) {?>
                ...
                ?>
                <small>(<a href="javascript: void(0);" onclick="javascript:getListBrands(this, <?= $dialog ?>); return false;"><?= GetMessage('CHANGEy') ?></a>)</small>
            <? } else { ?>
                <? ob_start() ?>
...
<tr id="tr_model"<? if ($arParams['MODEL_ID'] == 0 && $arParams['BRAND_ID'] == 0) { ?> class="auto-info-hide-tr"<? } ?>>
    <td class="field-name" valign="top"><?= GetMessage('MODEL') ?>:</td>
    <td id="td_model">
        <div class="dialog-content" style="display: none;"></div>
...
<tr id="tr_modification"<?if($arParams['MODIFICATION_ID'] == 0 && $arParams['MODEL_ID'] == 0){?> class="auto-info-hide-tr"<?}?>>
    <td class="field-name" valign="top"><?= GetMessage('MODIFICATION') ?>:</td>
