...
if (CModule::IncludeModule('linemedia.auto') && CModule::IncludeModule("linemedia.autotecdoc")) {
	...
	if (!empty($_REQUEST['MODIFICATIONS_SET'])) {
		$api->changeModificationsSetId($_REQUEST['MODIFICATIONS_SET']);
	}

    if ($sAction !== false) {
        switch ($sAction) {
            case 'getBrand':               
                $response = $api->query('getBrands', $data = array());
                if (is_array($response) && $response['status'] === 'ok' && ($response['data']) > 0 ) {
                    $aBrandRes = $response['data'];
                }
                ...
                case 'getModel':
                    if ($iBrandID !== false) {
                        ...
                        $response = $api->query('getVehicleModels', $data = array('brand_id' => $iBrandID));
                        if (is_array($response) && $response['status'] === 'ok' && ($response['data']) > 0 ) {
                            $aModels = $response['data'];
                        }
