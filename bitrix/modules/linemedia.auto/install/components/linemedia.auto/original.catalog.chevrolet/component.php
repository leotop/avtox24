...
/*
 * Учитывать ли бренд в ссылке на поиск
 */
$arParams['NO_BRAND'] = $arParams['NO_BRAND'] ?: 'N';
...
*/
if(LinemediaAutoUserHelper::isRobot('LM_AUTO_ORIG_CAT_HIT'))
{
...
/*
* Дополнительная фильтрация из запроса по VIN
*/
$additional_url = '';

$available_additional_vars = array('body_code', 'engine_code', 'transm_code');

foreach($available_additional_vars AS $var_code) {
	if($_GET[$var_code]) {
		$$var_code = strval($_GET[$var_code]);
		$additional_url[] = $var_code . '=' . $$var_code;
	}
}
$additional_url = count($additional_url) ? '?' . join('&', $additional_url) : '';
$arResult['ADDITIONAL_URL'] = $additional_url;
...
if($page == 'part_details') {
    ...
    $article['search_url'] = LinemediaAutoUrlHelper::getPartUrl($searchInfo);
    ...
    */
    ...
    try {
		$data = $api->query('getOriginalChevroletArticles', array(
			'group_section_id' => $arVariables['GROUP_SECTION'],
			'body_code' => $body_code,
			'engine_code' => $engine_code,
			'transm_code' => $transm_code,
		));
	} catch (Exception $e) {
        $arResult['ERROR'] = $e->GetMessage();
        $this->IncludeComponentTemplate('error');
    ...
    */
    ...
    foreach ($articles as $key => $detail) {
        ...
        */
        ...
        $articles[$key]['search_url'] = LinemediaAutoUrlHelper::getPartUrl($searchInfo);
        ...
        */
        ...
        $articles[$key]['Description'] = str_replace('</BR>', "\n", $articles[$key]['Description']);
        /*
