...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => $_SERVER['DOCUMENT_ROOT'] . '/upload/linemedia.auto/pricelists/price_groupsearch/',
    ...
    public static function lastOccurance($heystack, $needle) {
    
        $first = 0;
        while (($last = strpos($heystack, $needle, $first)) !== false) {
            $first = $last + 1;
        }
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $file_path = str_replace(
                        substr(
                            $file_path,
                            self::lastOccurance($file_path, '/'),
                            self::lastOccurance($file_path, '.') - self::lastOccurance($file_path, '/') - 1
                        ),
                        $this->get_user_id(),
                        $file_path
                    );
