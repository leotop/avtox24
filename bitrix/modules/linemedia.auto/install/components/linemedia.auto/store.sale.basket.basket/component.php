...
$arParams['SORT_ITEM'] = $arParams['SORT_ITEM'] ?: 'DATE';
$arParams['SORT_DIRECTION'] = $arParams['SORT_DIRECTION'] ?: 'DESC';
$arParams['CHECK_BASKET_ITEMS'] = $arParams['CHECK_BASKET_ITEMS'] ?: 'N';
...
if (strlen($_REQUEST["BasketRemoveLM"]) > 0 || strlen($_REQUEST["BasketRefreshLM"]) > 0 || strlen($_REQUEST["BasketOrderLM"]) > 0 || strlen($_REQUEST["lm_action"]) > 0) {
	if (strlen($_REQUEST["lm_action"]) > 0) {
		...
		if ($id > 0) {
			...
			if ($arBasket = $dbBasketItems->Fetch()) {
				if ($_REQUEST['lm_action'] == 'delete' && in_array("DELETE", $arParams["COLUMNS_LIST"])) {
					CSaleBasket::Delete($arBasket['ID']);
				} elseif ($_REQUEST["lm_action"] == "shelve" && in_array("DELAY", $arParams["COLUMNS_LIST"])) {
					if ($arBasket["DELAY"] == "N" && $arBasket["CAN_BUY"] == "Y") {
						CSaleBasket::Update($arBasket['ID'], Array("DELAY" => "Y"));
					}
				} elseif ($_REQUEST["lm_action"] == "add" && in_array("DELAY", $arParams["COLUMNS_LIST"])) {
		...
		if (strlen($_REQUEST['BasketRemoveLM']) > 0) {
	...
	if (strlen($_REQUEST['BasketOrderLM']) > 0) {

		/*
		 * Дата: 05.09.13 16:25
		 * Кто: Назарков Илья
		 * Задача: 5022
		 * Пояснения: Проверяем каждый товар в корзине на наличие в нашей бд.
		 */
		...
		if ($arParams['CHECK_BASKET_ITEMS'] == 'Y') {
			$dbBasketItems = CSaleBasket::GetList(
				array(),
				array(
					"FUSER_ID" => CSaleBasket::GetBasketUserID(),
				...
				false,
				false,
				array("ID", "PRODUCT_ID", "NAME", "DETAIL_PAGE_URL", "MODULE")
			);
			while ($arBasket = $dbBasketItems->Fetch()) {

				/*
				 * For parts from iblocks
				 */
				if ($arBasket['MODULE'] == 'catalog') {
					continue;
				}
				
				$part = new LinemediaAutoPart($arBasket['PRODUCT_ID']);
				if (!$part->exists()) {

					ShowError(GetMessage('ERROR'). '!');
					echo GetMessage('ERROR_ITEM', array('#NAME#' => $arBasket['NAME'], '#URL#' => $arBasket['DETAIL_PAGE_URL']));
		...
		unset($_REQUEST['BasketRemoveLM']);
		unset($_REQUEST['BasketRefreshLM']);
		unset($_REQUEST['BasketOrderLM']);
...
$dbBasketItems = CSaleBasket::GetList(
	...
	false,
	false,
	array("ID", "NAME", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE", "WEIGHT", "DETAIL_PAGE_URL", "NOTES", "CURRENCY", "VAT_RATE", "DATE_INSERT", "DATE_UPDATE")
);
while ($arItems = $dbBasketItems->GetNext()) {
	...
	//if (in_array("PROPS", $arParams["COLUMNS_LIST"])) {
        // грузим свойства всегда - нужны для учета кратности
...
for ($i = 0; $i < count($arBasketItems); $i++) {
    ...
    $currency_decimal = CCurrencyLang::GetCurrencyFormat($allCurrency);
	
    if($allCurrency != $arBasketItems[$i]["CURRENCY"]) {
        $arBasketItems[$i]["PRICE"] = CCurrencyRates::ConvertCurrency($arBasketItems[$i]["PRICE"], $arBasketItems[$i]["CURRENCY"], $allCurrency, $arBasketItems[$i]["DATE_INSERT"]);
    }
	...
	$arBasketItems[$i]["PRICE_FORMATED"] =  SaleFormatCurrency($arBasketItems[$i]["PRICE"], $allCurrency);
	...
	if ($arBasketItems[$i]["DELAY"] == "N" && $arBasketItems[$i]["CAN_BUY"] == "Y") {
		...
		$integer_array = $currency_decimal['DECIMALS'] == 0 ? (int) preg_replace('/[^\d]/', '', $arBasketItems[$i]["PRICE_FORMATED"]) : $arBasketItems[$i]["PRICE"];

		$allSum += $integer_array*$arBasketItems[$i]["QUANTITY"];
	...
	if ($bShowReady) {
		if ($val["DELAY"] == "N" && $val["CAN_BUY"] == "Y") {
			if (DoubleVal($val["DISCOUNT_PRICE"]) > 0) {
                ...
                if($allCurrency != $arBasketItems[$i]["CURRENCY"]) {
                    $val["DISCOUNT_PRICE"] = CCurrencyRates::ConvertCurrency($val["DISCOUNT_PRICE"], $val["CURRENCY"], $allCurrency, $val["DATE_INSERT"]);
                }
...
if (count($arBasketItems) <= 0) {
    ...
    /*
     * Добавляем обработку события вывода компонента корзины,
     * позволяющего расширить функционал корзины
     */
    $events = GetModuleEvents("linemedia.auto", "OnShowBasketForm");
    while ($arEvent = $events->Fetch()) {
        $arResult["ADDITIONAL_HTML"] = ExecuteModuleEventEx($arEvent, array($iMember));
    }
}
...
foreach ($arResult["ITEMS"]["AnDelCanBuy"] as $key => $item) {
	$valueSort[$key] = str_replace(' ', '', $item["{$arParams['SORT_ITEM']}"]);
}

$sortDirection = $arParams['SORT_DIRECTION'] == 'SORT_ASC' ? SORT_ASC : SORT_DESC;
$sortType = $arParams['SORT_ITEM'] == 'PRICE' ? SORT_NUMERIC : SORT_STRING;
array_multisort($valueSort, $sortDirection, $sortType, $arResult["ITEMS"]["AnDelCanBuy"]);
