    ...
    <? if (count($arResult["ITEMS"]["AnDelCanBuy"]) > 0) { ?>
                            ...
                            foreach ($arBasketItems["PROPS"] as $val) {
                                ...
                                if ($val['CODE'] == 'need_vin') {
                                    ...
                                    if (!$val["VALUE"]) {
                                        ...
                                        if ($modification_id && CModule::IncludeModule('linemedia.autogarage') && $USER->IsAuthorized()) {
                                            ...
                                            foreach ($user_cars as $user_car) {
                                                if ($user_car['PROPERTY_MODIFICATION_ID_VALUE'] == $modification_id) {
                                                    if ($user_car['PROPERTY_VIN_VALUE']) {
                                                        $val["VALUE"] = $user_car['PROPERTY_VIN_VALUE'];
                                                    }
                                                    break;
                                                }
                                            }
        ...
        <div class="cart-ordering">
            <? if ($arParams["HIDE_COUPON"] != "Y") { ?>
                <div class="cart-code">
                    <input <? if (empty($arResult["COUPON"])): ?>onclick="if (this.value=='<?= GetMessage("SALE_COUPON_VAL") ?>')this.value=''" onblur="if (this.value=='')this.value='<?= GetMessage("SALE_COUPON_VAL") ?>'"<? endif; ?>
                           value="<? if (!empty($arResult["COUPON"])): ?><?= $arResult["COUPON"] ?><? else: ?><?= GetMessage("SALE_COUPON_VAL") ?><?endif; ?>" name="COUPON">
                </div>
            <? } ?>
