		...
		<tr>
			<td colspan="2" class="account">
				...
				<label for="PAY_CURRENT_ACCOUNT">
					...
					<img src="/bitrix/components/linemedia.auto/sale.order.ajax/templates/visual/images/logo-default-ps.gif" alt="" <?=($arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")?"class=\"active\"":"";?> />
					<div class="desc">
...
<script>
    top.$(document).ready(function(){
		...
		//for ie 11(click to paysystem)
		if(UserAgentString.indexOf('Trident/7.0') + 1){
			$('.account label > img').on('click', function(){
				$(this).closest('label').trigger('click');
			});

			$('.logo label > img').on('click', function(){
				$(this).closest('label').trigger('click');
			});
		}
	});
