...
include('lang/ru/user_search.php');
...
/*if (!$USER->IsAdmin()) {
    $lmUser_id = $USER->GetID();
    $oLmUser = $USER->GetByID($lmUser_id);
    if($lmUser = $oLmUser->Fetch()) {
        $arFilter["UF_DEALER_ID"] = $lmUser['UF_DEALER_ID'];
    }
}*/
...
{
    // проверим нужно ли ограничивать дополнительно по полю UF_MANAGER_ID
    ...
    if(array_key_exists('UF_MANAGER_ID', $user_fields)) {
        ...
        if(empty($arFilter['ID']) && !$USER->IsAdmin()) {
            $arFilter['ID'] = -1;
        }
    }
...
elseif($maxRole == LM_AUTO_MAIN_ACCESS_READ || $maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE)
{
}
