...
// заказ оформляем в дефолтной валюте
...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
    {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
        {
            ...
            if(IntVal($arUserResult["DELIVERY_LOCATION"]) > 0)
            {
                if (strlen($arUserResult["DELIVERY_ID"]) > 0 && strpos($arUserResult["DELIVERY_ID"], ":") !== false)
                {
					...
					/*Добавлено, чтобы работала служба доставки Почта россии (нужен не пустой $arOrderTmpDel["ITEMS"])*/
        ...
        while ($arProperties = $dbProperties->GetNext())
        {
            ...
            {
                // CSaleOrderUserPropsValue::GetList выдает результат отличающийся от CSaleOrderUserProps::DoLoadProfiles. в админке используется CSaleOrderUserProps::DoLoadProfiles
                // будем считать CSaleOrderUserProps::DoLoadProfiles более правильным
        ...
        if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
        {
            ...
            if($USER->IsAuthorized() && empty($arResult["ERROR"]))
            {
				...
				if(empty($_REQUEST['user']) && $manager_make_order == true && (in_array($managersID, $USER->GetUserGroupArray()) || in_array($director_group, $USER->GetUserGroupArray())))
				{
					...
					/*
					* При оформлении заказа для физических лиц
					*/
					...
					/*
					* При оформлении заказа для юридических лиц
					*/
	            ...
	            if(empty($arResult["ERROR"])) {
	               ...
	               $events = GetModuleEvents("linemedia.auto", "OnOrderComplete");
					...
					//Проставить привязку заказа к менеджеру
                ...
                if (empty($arResult["ERROR"]))
                {
	                ...
	                /*
					 * Добавить пересчет цены товаров в заказе, если он оформлялся менеджером за другого пользователя
					 */
                    ...
                    while ($arOrderProperties = $dbOrderProperties->Fetch())
                    {
                        ...
                        if (strlen($curVal) > 0)
                        {
                            ...
                            if ( $arOrderProperties["USER_PROPS"] == "Y" && IntVal($arUserResult["PROFILE_ID"])<=0 && IntVal($arUserResult["PROFILE_ID_new"])<=0)
                            {
								...
								if((int)$_REQUEST["user"] > 0 && $manager_make_order == true) //Если заказ оформляет менеджер на пользователя и выбран существующий пользователь
								{
									...
									{
										$profile_name = $userNames["0"]." ".$userNames["1"]." ".$userNames["2"]; //Составим название нового профиля, из реквеста если заполнили поля фио
									}
									else
									{
										$profile_name = $ar_new_user["NAME"]." ".$ar_new_user["SECOND_NAME"]." ".$ar_new_user["LAST_NAME"]; //или из пользователя
									}
																		
									if(strlen(trim($profile_name)) == 0)
									{
										$profile_name = $_REQUEST["user"]."_".randString(10); //если нигде нет фио, сгенерируем строку
									}
								...
								else //Если заказ оформляет пользователь сам на себя
								{
									$arFields = array(
                                        "NAME" => $arUserResult["PROFILE_NAME"],
                                        "USER_ID" => IntVal($USER->GetID()),
