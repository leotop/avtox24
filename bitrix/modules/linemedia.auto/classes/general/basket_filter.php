...
class LinemediaAutoBasketFilter
{
    ...
    protected $filter_users         = array();
     ...
     protected $filter_nbasket_props  = array();
	...
	public function setCustomField($customField)
	{
		if (is_array($customField)) {
			$this->filter_basket_props[$customField['CODE']] = (string) $customField['VALUE'];
		}
	}
	...
	{
		$this->filter_basket_props[$field] = $value;
	}
   ...
   //Выбрать заказы, НЕ с таким-то статусом 
    public function setNStatus($statuses)
    {
        ...
        foreach ($statuses as $status) {
            $status = (string) $status;
            if (strlen($status) > 0) {
                $this->filter_nbasket_props['status'] []= $status;
            }
        }
    }
    ...
    {
        $this->filter_basket_props['brand_title'] = $brand;
    }
    ...
    public function setUserId($user_id)
    {
        ...
        */
        ...
        if (!isset($this->filter_orders['USER_ID']))
            $this->filter_orders['USER_ID'] = $user_id;
        else {
            $this->filter_orders['USER_ID'] = array_intersect($this->filter_orders['USER_ID'], $user_id) ?: array(-1);//Иначе выведет всех пользователей
        }
    }
	...
	{
		$this->filter_orders['%NAME_SEARCH'] = (string) $buyer;
	}
    
	public function setUniversalArr($arBuyer) //когда фильтруем по нескольким пользователям одновременно
	{
		$this->filter_orders['%NAME_SEARCH'] = $arBuyer;
	}
    ...
    public function filter()
    {
        ...
        if (!empty($this->filter_nbasket_props) && !$this->is_empty) {
            $this->unionIds($this->filterNBasketProps());
            $this->is_filtered = true;
        }
    ...
    protected function filterBasketProps()
    {
        ...
        foreach ($this->filter_basket_props as $code => $value) {
            ...
            if(in_array(substr($value, 0, 1), array('>', '<', '='))) {
	            $modificator = substr($value, 0, 1);
	            $value = substr($value, 1);
            }
            
            $baskets = CSaleBasket::GetPropsList(array(), array('CODE' => $code, $modificator.'VALUE' => $value), false, false, array('BASKET_ID'));
    ...
    protected function filterNBasketProps()
    {
        $ids = array();
        $first = true;
        foreach ($this->filter_nbasket_props as $code => $value) {
            $items = array();
            if($code == "status") $t = "!";
            else $t = "";
            ...
            $baskets = CSaleBasket::GetPropsList(array(), array('CODE' => $code, $t.'VALUE' => $value), false, false, array('BASKET_ID'));
            while ($basket = $baskets->Fetch()) {
                $items []= (int) $basket['BASKET_ID'];
            }
            if ($first) {
    ...
    protected function unionIds($ids)
    {
        ...
        if (empty($this->ids)) {
            ...
            /*
             * Nazarkov Ilya 23.10.2014 task#13534
             */
            if(empty($this->ids)) {
                $this->ids = array(-1);
            }
    ...
    public function getFilterOrdersParam($param) {
        return $this->filter_orders[$param];
    }

    public function getFilterBasketPropsParam($param) {
        return $this->filter_basket_props[$param];
    }
}
