...
/**
* Класс отвечает за вывод административной отладки по функциям Автоэксперта
*/
class LinemediaAutoDebug
{
     ...
     * @param string $message Текст, который будет отображён в спойлере (скрытым)
     * @param string $priority Важность сообщения
     * @param string $file Файл с дебагом
     * @param int $line Нмер строки с дебагом
     */
    public static function add($message, $spoiler = false, $priority = LM_AUTO_DEBUG_NOTICE, $file = false, $line = false) 
    {
        ...
        /**
            HTTP_USER_AGENT не установлен для консольного запуска.
            Если у нас что-то запущено с консоли, то никаких отладок,кроме записи в файл, мы не делаем.
            ибо сие кушает оперативку.
        */
        ...
        if ($_GET['lm_auto_debug'] != 'Y' || !is_object($USER) || !$USER->IsAdmin()) {
            ...
            /*
             * параметр right_debug позволит включить отладку всегда
             */
    ...
    /**
     * Отправка ошибки на почту при серьёзных ощибках
     */
    ...
    public static function addInformer($message, $spoiler, $file = false, $line = false, $priority = false)
    {
		 ...
		 * Кто: Назарков Илья
		 * Задача: 5051
		 * Пояснения: Убран информер на зеленом фоне
		 */
