...
class LinemediaAutoApiDriver
{
    ...
    */
    ...
    public function query($cmd, $data = array())
    {
		...
		/**
         * Событие обращения к апи
         */
        $events = GetModuleEvents("linemedia.auto", "OnBeginApiQuery");
        ...
        */
        ...
        if (isset($response_arr['status']) && $response_arr['status'] == 'error') {
	        ...
	        $events = GetModuleEvents("linemedia.auto", "OnApiQueryError");
	        while ($arEvent = $events->Fetch()) {
	            ExecuteModuleEventEx($arEvent, array(
	                &$cmd,
	                &$data,
         ...
         * Событие обращения к апи
         */
        $events = GetModuleEvents("linemedia.auto", "OnEndApiQuery");
        while ($arEvent = $events->Fetch()) {
            ExecuteModuleEventEx($arEvent, array(
                &$cmd,
                &$data,
