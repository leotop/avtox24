...
*/
    ...
    public function applyModifications($cmd, $args, &$response)
    {
    	switch ($cmd) {
	    	...
	    	case 'getDetailBrands':
	    		...
	    		foreach ($modifications as $mod) {
		    		...
		    		if ($data[self::LOCAL_ID_KEY] == 'Y') {
		    		    ...
		    		    $data['id']         = (int) $mod['id'];
                        $data['source_id']  = (string) $mod['source_id'];
	    	...
	    	case 'getDetails2':
	    		...
	    		/*
	    		* Используется на странице детального просмотра запчасти
	    		*/
	    		case 'getArticleDetailsMultiple':

	    			foreach($args AS $k => $part_args) {
		    			$modifications = $this->getModificationsList('part', $part_args['article_id']);
		    			foreach ($modifications as $mod) {
			    			/*
				    		 * Расшифруем изменение
				    		 */
				    		...
				    		if ($data[self::LOCAL_ID_KEY] == 'Y') {
		                        ...
		                        // заполняем поля
		                        if($data['articleNo']) {
			                        $response[$k]['assignedArticle']['articleNo'] = $data['articleNo'];
						    		$response[$k]['directArticle']['articleNo'] = $data['articleNo'];
		                        }

		                        if($data['brandName']) {
			                        $response[$k]['directArticle']['brandName'] = $data['brandName'];
		                        }

		                        if($data['genericArticleName']) {
			                        $response[$k]['directArticle']['articleName'] = $data['genericArticleName'];
		                        }
					    		...
					    		foreach ($response['brands'] as $i => $item) {
						    		if ($item['manuId'] == $source_id) {
							    		...
							    		$response[$k]['brands'][$i] = array_replace_recursive($item, $data);
							    		break;
						    		}
					    		}
				    		}
    ...
    public function addModification($type, $source_id, $parent_id, $data)
    {
        ...
        $events = GetModuleEvents("linemedia.auto", "OnBeforeTecdocModificationAdd");
