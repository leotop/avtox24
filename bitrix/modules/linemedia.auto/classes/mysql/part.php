...
class LinemediaAutoPart extends LinemediaAutoPartAll
{
    ...
    public function setQuantity($quantity)
    {
        ...
        try {
            ...
            LinemediaAutoDebug::add('Error updating part quantity (ID ' . $this->part_id . ') ' . $e->GetMessage());
        }
    }
    ...
    public function load()
    {
        ...
        if ($this->part_id > 0) {
            ...
            if($this->brand_title) {
                $where['brand_title'] = $this->brand_title;
            }
        }
        if($this->extra) $where = $where + (array) $this->extra;
    ...
    public function add()
    {
        $this->data['article'] = LinemediaAutoPartsHelper::clearArticle($this->data['article']);
    ...
    public function groupSearchIdentity() {

        $price = preg_replace("/[^0-9,.]/", "", $this->data['price']);

        $identity = array(
            ToLower($this->data['article']),
            ToLower($this->data['brand_title']),
            intval($price),
            intval($this->data['quantity']),
            ToLower($this->data['supplier_id']),
            intval($this->data['delivery']),
        );
