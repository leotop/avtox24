...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions>="U" && check_bitrix_sessid())
{
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/bitrix/admin/linemedia.auto_account.php?lang=".LANG.GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$account = CSaleUserAccount::GetList(
    array(),
    array("ID" => $ID),
    false,
    false,
...
if (CModule::IncludeModule("linemedia.autobranches")) {
	if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_FINANCE_BRANCH) == 0 && $ID != 0) {
	    ...
	    if (!in_array($account['USER_ID'], $usersID)) {
	        ShowError(GetMessage('LM_AUTO_ACCOUNT_EDIT_DENIED'));
	        return;
	    }
	...
	if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_FINANCE_CLIENTS) == 0 && $ID != 0) {
	    $usersID = \LinemediaAutoBranchesManager::getUsersOfManager($USER->GetID());
	    if (!in_array($dbAccount['USER_ID'], $usersID)) {
	        ShowError(GetMessage('LM_AUTO_ACCOUNT_EDIT_DENIED'));
	        return;
	    }
	}
...
$aMenu = array(
		array(
				...
				"LINK" => "/bitrix/admin/linemedia.auto_account.php?lang=".LANG.GetFilterParams("filter_"),
				"ICON"	=> "btn_list",
...
if ($ID > 0 && $saleModulePermissions >= "U")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/bitrix/admin/linemedia.auto_account_edit.php?lang=".LANG.GetFilterParams("filter_"),
			"ICON"	=> "btn_new",
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("SAEN_DELETE_ACCOUNT_CONFIRM")."')) window.location='/bitrix/admin/linemedia.auto_account.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."#tb';",
				"WARNING" => "Y",
				"ICON"	=> "btn_delete"
			);
	}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/bitrix/admin/linemedia.auto_account.php?lang=".LANG.GetFilterParams("filter_")
			)
	);
?>
