...
function setPageSettings(&$arPageSettings) {
    $arPageSettings = array(
        'ADMIN_PAGE' => 'linemedia.auto_buyers_list.php',
        'EDIT_PAGE' => 'linemedia.auto_buyers_list_edit.php',
        'SHOW_GROUP_TABS' => false,
    );
}
AddEventHandler("linemedia.auto", "OnBeforeUserEditPageBuild", 'setPageSettings');

function onBeforeUserSave_setGroupParam(&$arFields, $maxRole) {
    ...
    if($maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE_OWN_BRANCH || $maxRole == LM_AUTO_MAIN_ACCESS_READ_OWN_BRANCH) {
        ...
        if(CModule::IncludeModule('linemedia.autobranches')) {
            ...
            $dealer = LinemediaAutoBranchesDealer::getUserDealer($USER->GetID());

            if($dealer) {
                $arFields['UF_DEALER_ID'] = $dealer->getField('ID');
            }
        }
    }
    ...
    if(is_array($arFields["GROUP_ID"])) {
        $arFields["GROUP_ID"][] = $managersID;
    } else {
        $arFields["GROUP_ID"] = array(0 => $managersID);
    }*/
}
AddEventHandler("linemedia.auto", "OnBeforeUserSave", 'onBeforeUserSave_setGroupParam');
...
/**************************************/
	...
	$curUserGroup = $USER->GetUserGroupArray(); //массив групп пользовател€

	$maxRole = LinemediaAutoGroup::getMaxPermissionId($sModuleId, $curUserGroup, $arTasksFilter); //максимальна€ роль пользовател€
	...
	/**************************************/		
	$userIds = array();
	$flagResult = false;  // зачем это?
	if($maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE_OWN_CLIENTS)
	{
		$arClientsIds = LinemediaAutoGroup::getUserClients();
		if(strlen($arFilterOld["ID"]) > 0 && in_array($arFilterOld["ID"], $arClientsIds)) //≈сли в пользовательском фильтре пришел ид клиента и этот ид есть среди клиентов менеджера  	
		{
		...
		{
			$flagResult = false; //иначе устанавливаем флаг в false, чтобы в следующем гет листе не выбрались все подр€д пользователи
		}
		else //если в пользовательском фильтре ничего не пришло
		{
			...
			if(empty($userIds)) $flagResult = false; //если ни одного своего клиента не найдено, не позвол€ем делать дальнейшую выборку
	...
	elseif($maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE_OWN_BRANCH || $maxRole == LM_AUTO_MAIN_ACCESS_READ_OWN_BRANCH)
	{
		...
		(
			...
			"STRING_ID" 	=> "branch_%", //чтобы покупатель состо€л в том же филиале, что и текущий пользователь 
		);
	...
	elseif($maxRole == LM_AUTO_MAIN_ACCESS_READ || $maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE || $maxRole == LM_AUTO_MAIN_ACCESS_FULL)
	{
		if(strlen($arFilterOld["ID"]) > 0) //≈сли в пользовательском фильтре пришел ид покупател€, устанавливаем фильтр  	
		{
			$arFilter["ID"] = $arFilterOld["ID"];
		}
...
/*****************************************************  од файла /bitrix/modules/main/admin/user_edit.php **********************************************************/
...
/*
 * ѕеречень кастомного кода
 * 1. $arPageSettings массив, содержащий константы и замены по коду мспользовани€ этих констант
 * 2. событие обработки полей перед сохранением/добавлением OnBeforeUserSave
 */
...
$arPageSettings = array(
    'ADMIN_PAGE' => 'linemedia.autochief_user_admin.php',
    'EDIT_PAGE' => 'linemedia.autochief_user_edit.php',
    'SHOW_GROUP_TABS' => (($USER->CanDoOperation('view_subordinate_users') || $USER->CanDoOperation('view_all_users')) && $canSelfEdit),
);
$events = GetModuleEvents('linemedia.auto', 'OnBeforeUserEditPageBuild');
while ($arEvent = $events->Fetch()) {
    ExecuteModuleEventEx($arEvent, array(&$arPageSettings));
}

define("HELP_FILE", "users/user_edit.php");
$strRedirect_admin = BX_ROOT."/admin/".$arPageSettings['ADMIN_PAGE']."?lang=".LANG;
$strRedirect = BX_ROOT."/admin/".$arPageSettings['EDIT_PAGE']."?lang=".LANG;
...
if($USER->CanDoOperation('edit_subordinate_users') && !$USER->CanDoOperation('edit_all_users'))
{
	...
	{
		$arClientsIds = LinemediaAutoGroup::getUserClients();
		if(strlen($_REQUEST["ID"]) > 0 && !in_array($_REQUEST["ID"], $arClientsIds)) //≈сли в пользовательском фильтре пришел ид клиента и этот ид есть среди клиентов менеджера  	
		{
			if (count(array_diff($arUserGroups, $arUserSubordinateGroups)) > 0 && !$selfEdit)
				LocalRedirect(BX_ROOT."/admin/".$arPageSettings['ADMIN_PAGE']."?lang=".LANG);
		}
	...
	elseif($maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE_OWN_BRANCH || $maxRole == LM_AUTO_MAIN_ACCESS_READ_OWN_BRANCH)
	{
		...
		if(strlen($_REQUEST["ID"]) > 0 && !in_array($_REQUEST["ID"], $userIds)) //≈сли в пользовательском фильтре пришел ид клиента и этот ид есть среди клиентов менеджера  	
		{
			if (count(array_diff($arUserGroups, $arUserSubordinateGroups)) > 0 && !$selfEdit)
				LocalRedirect(BX_ROOT."/admin/".$arPageSettings['ADMIN_PAGE']."?lang=".LANG);
		}
...
$showGroupTabs = $arPageSettings['SHOW_GROUP_TABS'];
...
/************************************************************** START ************************************************************************************************************/
        // наше кастомное событие перед сохранением Ivan, 22.04.14

        $events = GetModuleEvents('linemedia.auto', 'OnBeforeUserSave');
        while ($arEvent = $events->Fetch()) {
            ExecuteModuleEventEx($arEvent, array(&$arFields, $maxRole));
        } 
/**************************************************************** END **********************************************************************************************************/

        // это корректнее делает событие
       //$arFields["UF_DEALER_ID"] = LinemediaAutoUser::getUserBranchByManagerId($USER->GetId());
       $arFields["UF_MANAGER_ID"] =  $USER->GetId();
...
if($canViewUserList)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/bitrix/admin/".$arPageSettings['ADMIN_PAGE']."?lang=".LANGUAGE_ID."&set_default=Y",
		"ICON"	=> "btn_list",
...
if($USER->CanDoOperation('edit_all_users') || $USER->CanDoOperation('edit_subordinate_users'))
{
	if ($ID>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/bitrix/admin/".$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new",
			"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
		);
		$aMenu[] = array(
			"TEXT"	=> GetMessage("MAIN_COPY_RECORD"),
			"LINK"	=> "/bitrix/admin/".$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID.htmlspecialcharsbx("&COPY_ID=").$ID,
			"ICON"	=> "btn_copy",
		...
		if ($ID!=1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/bitrix/admin/".$arPageSettings['ADMIN_PAGE']."?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
				"ICON"	=> "btn_delete",
...
$events = GetModuleEvents('linemedia.auto', 'OnAfterUserEditPageBuild');
while ($arEvent = $events->Fetch()) {
    $eventRes = ExecuteModuleEventEx($arEvent, array(&$maxRole));
}

//Add user fields tab only when there is fields defined or user has rights for adding new field
if(
    ((count($USER_FIELD_MANAGER->GetUserFields($PROPERTY_ID)) > 0) ||
	($USER_FIELD_MANAGER->GetRights($PROPERTY_ID) >= "W")) && $eventRes != 'NO_USER_SETTINGS'
)
{
	$tabControl->BeginNextFormTab();
...
if($canViewUserList)
{
	$tabControl->Buttons(array(
		"disabled" => !$editable,
		"btnSaveAndAdd" => true,
		"back_url" => $arPageSettings['ADMIN_PAGE']."?lang=".LANGUAGE_ID,
	));
}
else
{
