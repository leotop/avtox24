...
if ($ID != '' && $modulePermissions >= "W" && strcmp($userPermission, LM_AUTO_MAIN_ACCESS_READ) != 0)
{
    ...
    <? } ?>
        <tr class="adm-detail-required-field">
            ...
            <td width="60%">
                ...
                <input type="text" <?= strcmp($userPermission, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> name="group" value="<?= htmlspecialcharsEx($ID) ?>" size="40">
            </td>
        </tr>
    <tr class="adm-detail-required-field">
        <td width="40%" valign="top"><?= GetMessage("LM_AUTO_WORDFORMS_BRAND_TITLES") ?>:</td>
        <td width="60%" valign="top">
            <div class="wordforms">
            <? if (count($group) == 0) $group[] = '';?>
            <? foreach ($group as $wordform) { ?>
                <div class="wordform">
                    <input type="text" <?= strcmp($userPermission, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> name="brand_titles[]" value="<?= htmlspecialcharsEx($wordform) ?>" />
                    <input type="button" <?= strcmp($userPermission, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> class="wordform-del" value="&ndash;" />
                </div>
            <? } ?>
            </div>
            <input type="button" <?= strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> class="wordform-add" value="+" />
        </td>
    </tr>
...
if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_READ)) {
