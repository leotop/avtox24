...
function setPageSettings(&$arPageSettings) {
    ...
    $arPageSettings = array(
        ...
        'EDIT_PAGE' => 'linemedia.auto_managers_edit.php',
        'DROP_DOWN_FILTER' => 'AND ID=' . intval($managersID),
        'PAGE_TITLE' => GetMessage("LM_AUTO_MANAGERS_TITLE"),
    );
}
AddEventHandler("linemedia.auto", "OnBeforeUserAdminPageBuild", 'setPageSettings');

function onBeforeUsersListLoaded_setGroupFilter(&$arFilter) {

    $managersID = COption::GetOptionInt('linemedia.autobranches', 'LM_AUTO_BRANCHES_USER_GROUP_MANAGERS');

    $arFilter["GROUPS_ID"] = array($managersID);
}
AddEventHandler("linemedia.auto", "OnBeforeUsersListLoaded", 'onBeforeUsersListLoaded_setGroupFilter');
 ...
 * 2. событие обработки фильтра OnBeforeUsersListLoaded
 */
...
$events = GetModuleEvents('linemedia.auto', 'OnBeforeUsersListLoaded');
while ($arEvent = $events->Fetch()) {
...
if(CModule::IncludeModule('linemedia.autobranches')) {
	...
	try {
		$user = new LinemediaAutoBranchesUser($USER->GetID());
		$branch = $user->getBranch();
		$directorID = $branch->getDirectorID();
	} catch (Exception $e) {
		ShowError('Error: ',  $e->getMessage(), "\n");
	}

	if(is_object($branch) && $directorID == $USER->GetID()) {

		$propManagers = $branch->getProperty('managers');

		if(!empty($propManagers['VALUE'])) {
			$arFilter['ID'] = implode('|', $propManagers['VALUE']);
		}
	}
}

unset($arFilter['CHECK_SUBORDINATE']);
unset($arFilter['CHECK_SUBORDINATE_AND_OWN']);
...
while($arRes = $rsData->NavNext(true, "f_"))
{
    ...
    {
        $selfEdit = ($USER->CanDoOperation('edit_own_profile') && $f_ID == $USER->GetID());

        $arUserGroups = CUser::GetUserGroup($f_ID);
