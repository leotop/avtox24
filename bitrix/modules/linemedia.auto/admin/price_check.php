...
CUtil::InitJSCore(array('window', 'ajax', 'jquery'));
	    ...
	    case 'parts':
	        ...
	        foreach ($parts as $group_id => $sparts) {
	            ...
	            foreach ($sparts as $i => $part) {
                    ...
                    $c = $price->getRetailChain();
	                
	                $parts[$group_id][$i]['price_debug'] = $price->getDebug();

					/*
					 * Добавляем информацию о изменении вида цены в соотв. с настройками валюты
					 */
					array_pop ($parts[$group_id][$i]['price_debug']);
					$parts[$group_id][$i]['price_debug'][] = GetMessage('LM_AUTO_PRICE_CURRENCY_SETTINS', array('#PRICE#' => $parts[$group_id][$i]['price'], '#CURRENCY#' => $price->getCurrency()));
					$parts[$group_id][$i]['price_debug'][] = GetMessage('LM_AUTO_PRICE_FINAL_PRICE', array('#PRICE#' => $parts[$group_id][$i]['price']));
	        ...
	        $parts = LinemediaAutoPartsHelper::sortCatalogs($parts, 'price', 'asc');
            ...
            $branch_iblock_id = false;
            if(CModule::IncludeModule('linemedia.autobranches')) $branch_iblock_id = COption::GetOptionInt('linemedia.autobranches', 'LM_AUTO_IBLOCK_BRANCHES')
	        ...
	        <?foreach ($parts as $group_id => $parts) {?>
	        	...
	        	<?foreach ($parts as $i => $part) {?>
	        		<tr>
	        			...
	        			<td class="price"><nobr><?=$part['price']?></nobr></td>
	        			<td class="debug">
	        				...
	        				<?
                                ...
                                $branch_url = '';
                                if($branch_iblock_id) $branch_url = '/bitrix/admin/iblock_element_edit.php?IBLOCK_ID='.$branch_iblock_id.'&type=linemedia_auto&lang='.LANG.'&ID=';
	        					...
	        					foreach($part['price_debug'] AS $d)
	        					{
                                    ...
                                    $d = str_replace('[[BRANCH_URL]]', $branch_url, $d);
...
<form action="" method="POST" name="price_check_frm" id="price_check_frm">
	...
	<div class="user">
		<?echo FindUserID("user", CUser::GetId(), '', "price_check_frm", 4)?>
...
<script>
	...
	$('#price_check_frm').submit(function(){
		$('#brand_title_inp').removeAttr('value');
		$('#extra_inp').removeAttr('value');
