...
<? foreach ($check as $module_id => $checks) { ?>
	<thead>
		<tr class="module">
			<th colspan="3"><?=GetMessage('LM_AUTO_CHECK_MODULE')?> <span><?=LinemediaAutoModule::getModuleTitle($module_id)?></span></th>
		</tr>
		<tr class="titles">
	...
	<tbody>
		<?foreach($checks AS $check){?>
			<tr class="check check-<?=$check['status']?'ok' : ($check['recomendation'] ? 'recomendation' : 'fail')?>">
				<td><?=$check['title']?><?=($check['recomendation']) ? GetMessage('STATUS_CHECK_RECOMENDATION') : ''?></td>
				<td>
					<?if(!$check['status']){?>
					<?=$check['requirements']?>
					<?}?>
...
<table class="lm-auto-check lm-auto-check-logs">
	...
	<? foreach ($logs AS $module_id => $module_logs){
		$title = LinemediaAutoModule::getModuleTitle($module_id);
