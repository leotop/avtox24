...
function setPageSettings(&$arPageSettings) {
    $arPageSettings = array(
        'ADMIN_PAGE' => 'linemedia.auto_managers.php',
        'EDIT_PAGE' => 'linemedia.auto_buyers_list_edit.php',
        'DROP_DOWN_FILTER' => 'AND ID!=2',
        'PAGE_TITLE' => GetMessage("LM_AUTO_BUYERS_TITLE"),
    );
}

AddEventHandler("linemedia.auto", "OnBeforeUserAdminPageBuild", 'setPageSettings');

/*****************************************************  од файла /bitrix/modules/main/admin/user_admin.php **********************************************************/
 ...
 * 1. $arPageSettings массив, содержащий константы и замены по коду использовани€ этих констант
 * 2. событие обработки фильтра OnBeforeBuyersListLoaded
 */
...
$events = GetModuleEvents('linemedia.auto', 'OnBeforeUserAdminPageBuild');
...
if($_REQUEST["action"] == "authorize" && check_bitrix_sessid() && $USER->CanDoOperation('edit_php'))
{
	...
	LocalRedirect($arPageSettings['ADMIN_PAGE'] . "?lang=".LANGUAGE_ID);
}
...
if($handle_subord)
{
    ...
    //$arFilter["GROUPS_ID"] = $arUserSubordinateGroups;
...
/************************************************************** START ************************************************************************************************************/
// наше кастомное событие обработки фильтра Ivan, 22.04.14

$events = GetModuleEvents('linemedia.auto', 'OnBeforeBuyersListLoaded');
while ($arEvent = $events->Fetch()) {
    ExecuteModuleEventEx($arEvent, array(&$arFilter));
}
/**************************************************************** END **********************************************************************************************************/
...
{	
	if(is_array($userIds) && !empty($userIds))
		$arFilter2["ID"] = implode("|", $userIds);
	else $arFilter2["ID"] = $userIds;
	
	if($flagResult == true || $maxRole == LM_AUTO_MAIN_ACCESS_READ || $maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE || $maxRole == LM_AUTO_MAIN_ACCESS_FULL)
	{
		if($maxRole == LM_AUTO_MAIN_ACCESS_READ || $maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE || $maxRole == LM_AUTO_MAIN_ACCESS_FULL)
		{
		...
		{
			$rsData = CUser::GetList($by, $order, $arFilter2, array(
				"SELECT" => $lAdmin->GetVisibleHeaderColumns(),
				"NAV_PARAMS"=> array("nPageSize"=>CAdminResult::GetNavSize($sTableID)),
			));
		}
...
if($flagResult == true || $maxRole == LM_AUTO_MAIN_ACCESS_READ || $maxRole == LM_AUTO_MAIN_ACCESS_READ_WRITE || $maxRole == LM_AUTO_MAIN_ACCESS_FULL)
{
	...
	{
        LinemediaAutoDebug::add('list filter ' . print_r($arFilter2, true));
		$rsData = CUser::GetList($by, $order, $arFilter2, array(
			"SELECT" => $lAdmin->GetVisibleHeaderColumns(),
			"NAV_PARAMS"=> array("nPageSize"=>CAdminResult::GetNavSize($sTableID)),
		));
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	$row->AddViewField("ID", "<a href='".$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID."&ID=".$f_ID."' title='".GetMessage("MAIN_EDIT_TITLE")."'>".$f_ID."</a>");
	...
	if ($can_edit && $edit)
	{
		$row->AddField("LOGIN", "<a href='".$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID."&ID=".$f_ID."' title='".GetMessage("MAIN_EDIT_TITLE")."'>".$f_LOGIN."</a>", true);
	...
	{
		$row->AddViewField("LOGIN", "<a href='".$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID."&ID=".$f_ID."' title='".GetMessage("MAIN_EDIT_TITLE")."'>".$f_LOGIN."</a>");
	...
	$arActions = Array();
	$arActions[] = array("ICON"=>$can_edit ? "edit" : "view", "TEXT"=>GetMessage($can_edit ? "MAIN_ADMIN_MENU_EDIT" : "MAIN_ADMIN_MENU_VIEW"), "LINK"=> $arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID."&ID=".$f_ID, "DEFAULT"=>true);
	if($can_edit && $edit)
	{
		$arActions[] = array("ICON"=>"copy", "TEXT"=>GetMessage("MAIN_ADMIN_ADD_COPY"), "LINK"=>$arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID."&COPY_ID=".$f_ID);
		if (!$own_edit)
	...
	if($USER->CanDoOperation('edit_php'))
	{
		...
		$arActions[] = array("ICON"=>"", "TEXT"=>GetMessage("MAIN_ADMIN_AUTH"), "TITLE"=>GetMessage("MAIN_ADMIN_AUTH_TITLE"), "LINK"=>$arPageSettings['ADMIN_PAGE']."?lang=".LANGUAGE_ID."&ID=".$f_ID."&action=authorize&".bitrix_sessid_get());
	}
...
if ($USER->CanDoOperation('edit_subordinate_users') || $USER->CanDoOperation('edit_all_users'))
{
	...
	$aContext[] = array(
		...
		"LINK"	=> $arPageSettings['EDIT_PAGE']."?lang=".LANGUAGE_ID,
...
$APPLICATION->SetTitle($arPageSettings['PAGE_TITLE']);

require($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/main/include/prolog_admin_after.php");

CAdminMessage::ShowMessage($err); 
if($maxRole > LM_AUTO_MAIN_ACCESS_DENIED)
{	
	?>
	...
	<tr valign="top">
		...
		<td><?
		$z = CGroup::GetDropDownList($arPageSettings['DROP_DOWN_FILTER']);
	...
	<?
	$lAdmin->DisplayList();
} //end $maxRole > LM_AUTO_MAIN_ACCESS_DENIED
