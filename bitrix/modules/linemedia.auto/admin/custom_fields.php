...
//use Linemedia\Auto\BuildUpAdminListInterface\AdminOwnListOnUserRights as AdminOwnListOnUserRights;
...
$userPermissions = \LinemediaAutoGroup::getMaxPermissionId('linemedia.auto', $USER->GetUserGroupArray(), array('BINDING' => LM_AUTO_ACCESS_BINDING_CUSTOM_FIELDS));
...
<form  action="" method="POST" name="custom_fields_frm" id="custom_fields_frm">
                    ...
                    <tbody>
                        ...
                        <? foreach ($arCustomFields as $arCustomField) { ?>
                            ...
                            <tr>
                                ...
                                <td style="text-align: center; vertical-align: middle;">
                                    <input type="text" <?= strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> "size="25" maxlength="255" name="CUSTOM_FIELDS_CODE[<?= $cid ?>]" id="CUSTOM_FIELD_CODE_<?= $cid ?>" value="<?= $arCustomField['code'] ?>" />
                                </td>
                                <td style="text-align: center; vertical-align: middle;">
                                    <input type="text" <?= strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> size="25" maxlength="255" name="CUSTOM_FIELDS_NAME[<?= $cid ?>]" id="CUSTOM_FIELD_NAME_<?= $cid ?>" value="<?= $arCustomField['name'] ?>" />
                                </td>
                                <td style="text-align: center; vertical-align: middle;">
                                    <textarea cols="25" <?= strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> name="CUSTOM_FIELDS_DESCRIPTION[<?= $cid ?>]" id="CUSTOM_FIELDS_DESCRIPTION_<?= $cid ?>"><?= $arCustomField['description'] ?></textarea>
                                </td>
                                <td style="text-align: center; vertical-align: middle;">
                                    <select name="CUSTOM_FIELDS_TYPE[<?= $cid ?>]" style="width: 120px;" <?= strcmp($userPermissions, 'R') == 0 ? 'disabled' : '' ?>>
                                ...
                                <td style="text-align: center; vertical-align: middle;">
                                    <input type="checkbox" <?= strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) == 0 ? 'disabled' : ''; ?> name="CUSTOM_FIELDS_REMOVE[<?= $cid ?>]" id="CUSTOM_FIELDS_REMOVE_<?= $cid ?>" value="Y" class="adm-designed-checkbox" />
                                    <label class="adm-designed-checkbox-label" for="CUSTOM_FIELDS_REMOVE_<?= $cid ?>" title="<?= GetMessage('LM_AUTO_CF_CHECK_TO_REMOVE') ?>"></label>
                                </td>
                            </tr>
                        <? } ?>

                        <? if (strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ_WRITE) == 0 || strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_FULL) == 0) {
    ...
    if (strcmp($userPermissions, LM_AUTO_MAIN_ACCESS_READ) != 0) {
