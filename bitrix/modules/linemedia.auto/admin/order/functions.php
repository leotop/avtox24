...
/*
     ...
     * Создаём событие "Сокрытие свойств заказа"
     
    $events = GetModuleEvents("linemedia.auto", "OnHideUtilOrderProps");
    while ($arEvent = $events->Fetch()) {
        $arUtilFields = array_merge($arUtilFields, ExecuteModuleEventEx($arEvent, array()));
    }
    */
    $arUtilFields []= 'MODIFICATION_ID';
        ...
        if (count($arCountProps) < 3) {
            $resultHtml .= "<td colspan=2>
                    ...
                    if (IntVal($arCountProps["IS_PAYER"]) <= 0)
                        $resultHtml .= "<tr class=\"adm-detail-required-field\">
                            <td width=\"40%\" class=\"adm-detail-content-cell-l\">".GetMessage("NEWO_BUYER_REG_LASTNAME")."</td>
                        ...
                        <tr class=\"adm-detail-required-field\">
                            <td width=\"40%\" class=\"adm-detail-content-cell-l\">".GetMessage("NEWO_BUYER_REG_NAME")."</td>
						...
						<tr>
                            <td style=\"font-weight:normal !important;\" class=\"adm-detail-content-cell-l\" width=\"40%\">".GetMessage("NEWO_BUYER_REG_SECONDNAME")."</td>
                            <td class=\"adm-detail-content-cell-r\"><input type=\"text\" name=\"NEW_BUYER_SECOND_NAME\" size=\"30\" value=\"".htmlspecialcharsbx(trim($_REQUEST["NEW_BUYER_SECOND_NAME"]))."\" tabindex=\"2\" /></td>
                        </tr>";
    ...
    foreach ($arAllProperties as $arProperties) {
        ...
        /*
         * Сокрытие служебных полей.
         */
        if (in_array($arProperties['CODE'], $arUtilFields)) {
            continue;
        }
        ...
        if ($arProperties["TYPE"] == "CHECKBOX") {
            ...
            if ($arProperties["IS_PAYER"] == "Y" && IntVal($USER_ID) <= 0) {
                ...
                $resultHtml .= "<div class=\"fio newo_break_active\"><input style=\"color:black!important\" placeholder=\"".GetMessage('NEWO_BREAK_LAST_NAME')."\" type=\"text\" name=\"BREAK_LAST_NAME\"
                id=\"BREAK_LAST_NAME\" size=\"30\" value=\"".$BREAK_LAST_NAME_TMP."\" /></div>";
                $resultHtml .= "<div class=\"fio newo_break_active\"><input style=\"color:black!important\" placeholder=\"".GetMessage('NEWO_BREAK_NAME')."\" type=\"text\" name=\"BREAK_NAME\"
                id=\"BREAK_NAME_BUYER\" size=\"30\" value=\"".$NEWO_BREAK_NAME_TMP."\" /></div>";
                $resultHtml .= "<div class=\"fio newo_break_active\"><input style=\"color:black!important\" placeholder=\"".GetMessage('NEWO_BREAK_SECOND_NAME')."\" type=\"text\" name=\"BREAK_SECOND_NAME\"
                id=\"BREAK_SECOND_NAME\" size=\"30\" value=\"".$BREAK_SECOND_NAME_TMP."\" /></div>";
                $resultHtml .= '</div>';
