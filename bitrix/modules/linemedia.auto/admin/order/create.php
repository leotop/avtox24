 ...
 * Cоздаём событие
 */
$events = GetModuleEvents('linemedia.auto', 'OnBeforeOrdersNewPageBuild');
...
if ($ID > 0) {
    $ALLOW_EDIT_STATUSES = unserialize(COption::GetOptionString('linemedia.auto', 'LM_AUTO_MAIN_STATUSES_WHEN_CAN_EDIT_ORDERS', ''));
    $arOrder = CSaleOrder::GetByID($ID);
    if ( !in_array($arOrder['STATUS_ID'], $ALLOW_EDIT_STATUSES)) {
        $errorMessage = GetMessage('LM_AUTO_MAIN_CAN_NOT_EDIT_STATUS');
...
if ($REQUEST_METHOD == "POST" && $save_order_data == "Y" && empty($dontsave) && $saleModulePermissions >= "U" && check_bitrix_sessid()) {
        ...
        while ($arOrderProps = $dbOrderProps->Fetch()) {
            ...
            if ($arOrderProps["IS_PAYER"] == "Y") {
                if (strlen($curVal) <= 0) {
                    ...
                    $curVal = $NEW_BUYER_LAST_NAME." ".$NEW_BUYER_NAME." ".$NEW_BUYER_SECOND_NAME;
                }
    ...
    if ($btnNewBuyer == "Y" && strlen($errorMessage) <= 0) {
        ...
        if (strlen($NEW_BUYER_EMAIL) <= 0) {
            $errorMessage .= GetMessage("NEWO_BUYER_REG_ERR_MAIL")."<br>";
        }
    ...
    if (strlen($errorMessage) <= 0) {
        ...
        $arShoppingCart = (array) $lmbasket->getOrderedBaskets($ID);
    ...
    if (strlen($errorMessage) <= 0) {
        ...
        if (count($arOrder) > 0) {
             ...
             */
            $events = GetModuleEvents("linemedia.auto", $ID < 1 ? "OnBeforeOrderAdd":"OnBeforeOrderEdit");
            while ($arEvent = $events->Fetch()) {
                ExecuteModuleEventEx($arEvent, array(&$arOrder, &$arAdditionalFields));
            }

            /*
             * Иначе происходит задвоение корзин, сами корзины добавляются ниже
             */
             ...
             */
            $events = GetModuleEvents("linemedia.auto", $ID < 1 ? "OnAfterOrderAdd": "OnAfterOrderEdit");
            while ($arEvent = $events->Fetch()) {
            ...
            if ($ID <= 0) {
                $order  = new LinemediaAutoOrder($tmpID);
                $basket = new LinemediaAutoBasket();
                
                // Проставляем статусы товаров в статус заказа.
                $arOrderBaskets = $order->getBaskets();
                foreach ($arOrderBaskets as $arOrderBasket) {
                    $basket->statusItem($arOrderBasket['ID'], $arOrderFields['STATUS_ID']);
                }
            ...
            if ($ID > 0 && count($arErrors) <= 0 && $_POST["CANCELED"]) {
            ...
            if ($ID > 0 && count($arErrors) <= 0 && $_POST['PAYED']) {
            ...
            if ($ID > 0 && count($arErrors) <= 0 && $_POST["ALLOW_DELIVERY"]) {
    ...
    if (strlen($errorMessage) <= 0 && $ID > 0) {
        ...
        CSaleOrder::UnLock($ID);
        LocalRedirect("/bitrix/admin/".$arPageSettings['LIST_PAGE']."?lang=".LANG."&LID=".CUtil::JSEscape($LID));
...
if (!empty($dontsave)) {
    ...
    LocalRedirect("/bitrix/admin/".$arPageSettings['LIST_PAGE']."?lang=".LANG."&LID=".CUtil::JSEscape($LID).GetFilterParams("filter_", false));
}
...
if (isset($ORDER_AJAX) && $ORDER_AJAX == "Y" && check_bitrix_sessid()) {
     ...
     * Показ корзины.
     */
    /*if (isset($_REQUEST['ID']) AND !isset($_REQUEST['addcart']) AND !isset($_REQUEST['buyertypechange'])) {
        fGetBaskets($_REQUEST['ID']);
        die();
    } */
    ...
    if (isset($_REQUEST['checkhtml'])) {
        ...
        while ($arEvent = $events->Fetch()) {
            $html .= ExecuteModuleEventEx($arEvent, array($userID, $personID));
        }
        echo $html;
        die();
    }
    ...
    if (isset($_REQUEST['editcart']) && !empty($_REQUEST['id'])) {
        ...
        $result = $basket->Update($cid, array('QUANTITY' => $quantity));
    ...
    if (isset($_REQUEST['addcart']) && !empty($_REQUEST['cart'])) {
        ...
        if (isset($ID) && $ID > 0 && $basket_id > 0) {
            CSaleBasket::OrderBasket($ID, CSaleBasket::GetBasketUserID(), $LID);
        }
    ...
    /*
     * Получение местоположения для расчета доставки.
     */
    ...
    /*
     * Получение списка профилей и корзин пользователя.
     */
    ...
    if (isset($userId) && isset($buyerType) && isset($profileDefault)) {
        ...
        foreach ($arPropValues as $key => $val):
	        ...
	        $scriptExec .= "if(el)\n{\n";
	        $scriptExec .= "BX.ajax.post('/bitrix/admin/".$arPageSettings['NEW_PAGE']."', '".bitrix_sessid_get()."&ORDER_AJAX=Y&locid=".$key."&propID=".$buyerType."&LID=".CUtil::JSEscape($LID)."&location=".$val."', fLocationResult);\n";
	        $scriptExec .= "}";
        ...
        <div id="select_lid">
            <form action="" name="select_lid">
                ...
                <script type="text/javascript">
                    ...
                    {
                        BX.showWait();
                        BX.ajax.post("/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>", "<?=bitrix_sessid_get()?>&ORDER_AJAX=Y&lang=<?=LANGUAGE_ID?>&LID=" + el.value, fLidChangeResult);
                    }
...
$aMenu = array(
    array(
        ...
        "LINK" => "/bitrix/admin/".$arPageSettings['LIST_PAGE']."?lang=".LANGUAGE_ID
    )
);
...
if (($saleModulePermissions == "W" || $str_PAYED != "Y") && $bUserCanDeleteOrder && $ID > 0) {
    $aMenu[] = array(
            ...
            "LINK"  => "javascript:if(confirm('".GetMessage("NEWO_CONFIRM_DEL_MESSAGE")."')) window.location='".$arPageSettings['LIST_PAGE']."?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get().urlencode(GetFilterParams("filter_"))."'",
            "WARNING" => "Y"
        );
}
...
$events = GetModuleEvents("linemedia.auto", "OnShowOrderCreateForm");
while ($arEvent = $events->Fetch()) {
    $html .= ExecuteModuleEventEx($arEvent, array($str_USER_ID, $str_PERSON_TYPE_ID, $ID));
}
...
if (IntVal($ID) > 0) {
    ...
    if (count($arSitesShop) > 1) {
        ...
        <tr>
            ...
            <td width="60%">
                <?= htmlspecialcharsbx($arSitesShop[$str_LID]["NAME"])." (".$str_LID.")" ?>
            </td>
        </tr>
        <?
...
<tr>
    <td id="buyer_type_change" colspan="2">
		<input type="hidden" id="curr_user_id" name="curr_user_id">
		<?$str_USER_ID = ($_REQUEST['curr_user_id'] == 'NEW_USER') ? 0 : $str_USER_ID;?>
        ...
        function fButtonCurrent(el)
        {
            ...
            else if (el == 'btnBuyerExist' || el == 'btnBuyerExistRemote')
            {
                ...
                if (el == 'btnBuyerExist') {
                    var userSelect = window.open('/bitrix/admin/user_search.php?lang=<?= $lang ?>&FN=form_order_buyers_form&FC=user_id', '', 'scrollbars=yes,resizable=yes,width=840,height=500,' +
						'top='+Math.floor((screen.height - 560)/2-14)+',left='+Math.floor((screen.width - 840)/2-5));
                }
				 ...
				 * Кто: Назарков Илья
				 * Задача: 5678
				 * Пояснения: автоподстановка данных профиля
				 */
				 ?>
				$(userSelect).on('unload', function () {
					partA();
					function partA() {
						window.setTimeout(partB,750);
            }
					function partB() {
						fChangeProfile(document.getElementById("user_profile"));
						<?//Удаляем битрикосовый onclick, чтобы срабатывал наш обработчик?>
						$('div#user_name > a[onclick]').removeAttr("onclick");
        }
				});
		 ...
		 * Кто: Назарков Илья
		 * Задача: 5678
		 * Пояснения: автоподстановка данных профиля при выборе пользователя через битриксовый функционал
 		 * (клик на уже выбранном пользователе)
		 */
		 ?>
		$('div#user_name').live('click', function(event) {
			$('div#user_name').attr('onclick','').unbind( "click" );
			fButtonCurrent('btnBuyerExist');
		});
        ...
        function fBuyerChangeType(el)
        {
            ...
            BX.showWait();
            BX.ajax.post('/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>', '<?=bitrix_sessid_get()?>&ORDER_AJAX=Y&paysystemid=' + orderPaySysyemID + '&ID=' + orderID + '&LID=<?=CUtil::JSEscape($LID)?>&buyertypechange=' + el.value + '&userId=' + userId, fBuyerChangeTypeResult);
        }
        ...
        function fUserGetProfile(el)
        {
            ...
            if (userId != "" && buyerType != "") {
                BX.showWait();
                BX.ajax.post('/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>', '<?= bitrix_sessid_get() ?>&ORDER_AJAX=Y&id=<?= $ID ?>&LID=<?= CUtil::JSEscape($LID) ?>&currency=<?= $str_CURRENCY ?>&userId=' + userId + '&buyerType=' + buyerType, fUserGetProfileResult);
            }
            RefreshHTML(userId, buyerType);
			BX.closeWait();
        }
        ...
        {
            BX.ajax({
                url: '/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>',
                method: 'POST',
...
if (intval($ID) > 0) {
    ...
    $basket = new LinemediaAutoBasket();
    $arBaskets = $basket->getOrderedBaskets($ID);
...
$tabControl->AddSection("NEWO_HTML_EVENTS", GetMessage("NEWO_HTML_EVENTS"));

$tabControl->BeginCustomField("NEWO_HTML_EVENTS_X", GetMessage("NEWO_HTML_EVENTS"), true);

?>
<tr>
    <td colspan="2" id="module-html-id"><?= $html ?></td>
</tr>
<?

$tabControl->EndCustomField("NEWO_HTML_EVENTS_X");
        ...
        <table width="100%" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td align="right" nowrap>
                    <a title="<?= GetMessage("SOE_ADD_ITEMS_TITLE") ?>" onClick="AddProductSearch(<?= (count($arBaskets) + 1) ?>);" class="adm-btn adm-btn-green adm-btn-add"  style="white-space:nowrap;" href="javascript:void(0);">
                        <?= GetMessage("SOE_ADD_ITEMS") ?>
                    </a>
                </td>
            </tr>
        </table>
...
<tr>
    <td valign="top" align="left" colspan="2">
        ...
        <div style="float:right">
            ...
            <? $productAddBool = false;//COption::GetOptionString('sale', 'SALE_ADMIN_NEW_PRODUCT', 'N'); ?>
            <? if ($productAddBool == "Y") { ?>
            ...
            <? } ?>
            <a title="<?= GetMessage("SOE_ADD_ITEMS_TITLE") ?>" onClick="AddProductSearch(<?= (count($arBasket) + 1) ?>);" class="adm-btn adm-btn-green adm-btn-add" href="javascript:void(0);">
                <?= GetMessage("SOE_ADD_ITEMS") ?>
            </a>
        </div>
...
<script language="JavaScript">
    ...
    function RefreshBasket()
    {
        ...
        BX.showWait();
        BX.ajax.post('/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>', dateURL, RefreshBasketResult);
    }
    ...
    function FillProductFields(index, arParams, price)
    {
        ...
        BX.showWait();
        BX.ajax.post('/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>', dateURL, RefreshBasket);
    ...
    {
        $.ajax({
            url: '/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>?<?= bitrix_sessid_get() ?>&ORDER_AJAX=Y&delcart=Y&id=' + id,
     ...
     * Обновление HTML.
     */
    function RefreshHTML(userId, personId)
    {
        $.ajax({
            url: '/bitrix/admin/<?=$arPageSettings['NEW_PAGE']?>?<?= bitrix_sessid_get() ?>&ORDER_AJAX=Y&checkhtml=Y&userID=' + userId + '&personID=' + personId,
        }).done(function(response) {
...
if (!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1) {
    $tabControl->Buttons(array("back_url"=>"/bitrix/admin/".$arPageSettings['NEW_PAGE']."?lang=".LANGUAGE_ID."&ID=".$ID."&dontsave=Y"));
}
...
<div class="sale_popup_form" id="popup_form_sku_order" style="display:none;">
    ...
    <script>
		...
		$(document).ready(function() {
			$('#btnBuyerNew').live('click', function() {
				$('*:text').val('');
				$('#curr_user_id').val('NEW_USER');
				$('#user_id').val('0');
			})
			
			$('#btnBuyerExist').live('click', function() {			
				$('#curr_user_id').val('USER_EXIST');
			})
			
			if($('#user_name').find('a').html() != undefined && $('#user_name').find('a').html() != null && $('#btnBuyerExist').hasClass("adm-btn-active") == true)
				if($('#btnBuyerNewField').css('display') != 'none')
					$('#btnBuyerNewField').css({'display':'none'});
