...
$section = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $modificator_iblock_id, 'ACTIVE' => 'Y', 'CODE' => 'admin'), false, false, array())->GetNext();
...
//set up search modificator
$search->setModificator($section['CODE']);
	        ...
	        */
	        foreach ($arResult['PARTS'] as $group_id => $parts) {
	            foreach ($parts as $i => $part) {
					...
					//task 11371
		            if(!isset($part['price']) || !isset($part['article'])) {
	                ...
	                /*
	                 * Посчитаем цены для разных групп
	                */
