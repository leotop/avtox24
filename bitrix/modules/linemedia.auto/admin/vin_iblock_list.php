...
if (strcmp($saleModulePermissions, LM_AUTO_MAIN_ACCESS_DENIED) == 0) {
    $APPLICATION->AuthForm(GetMessage("ACCESS_DENIED"));
}

$userPermission = \LinemediaAutoGroup::getMaxPermissionId('linemedia.auto', $USER->GetUserGroupArray(), array('BINDING' => LM_AUTO_ACCESS_BINDING_VIN));
...
if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_VIN_CLIENTS) == 0) {
    $arFilter['PROPERTY_MANAGER'] = $USER->GetID();
    unset($FilterArr['find_MANAGER']);
}

$usersBranch = array();

if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_VIN_BRANCH) == 0) {  

    
    $directorId = array();
    $outcome = CIBlockElement::GetList(array(), array('IBLOCK_ID' => COption::GetOptionInt('linemedia.autobranches', 'LM_AUTO_IBLOCK_BRANCHES')), false, array(), array('PROPERTY_director'));
    
    while ($rs = $outcome->Fetch()) {
        $directorId[] = $rs['PROPERTY_DIRECTOR_VALUE'];
    }
    
    if (in_array($USER->GetID(), $directorId)) {
        
        $branchFeatures = CIBlockElement::GetList(array(), array('IBLOCK_ID' => COption::GetOptionInt('linemedia.autobranches', 'LM_AUTO_IBLOCK_BRANCHES'), 'PROPERTY_director' => $USER->GetID()))->Fetch();       
        $branch = new LinemediaAutoBranchesBranch($branchFeatures['ID']);
        $usersBranch = $branch->getBranchesUserIDsList();
    } 
}
...
while ($arRes = $rsData->NavNext(true, "f_")) {  
  ...
  if (strcmp($userPermission, LM_AUTO_MAIN_ACCESS_VIN_BRANCH) == 0 && !in_array($arRes['CREATED_BY'], $usersBranch)) {
      continue;
  }
...
while ($arEvent = $events->Fetch()) {
    ...
    if (ExecuteModuleEventEx($arEvent, array(&$lAdmin, \Linemedia\Auto\Privilege\PageID::VIN)) == FALSE) {
...
if(CModule::IncludeModule('linemedia.autobranches') && strcmp($userPermission, LM_AUTO_MAIN_ACCESS_VIN_CLIENTS) != 0) {
