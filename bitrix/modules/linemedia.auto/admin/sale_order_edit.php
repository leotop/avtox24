 ...
 * Cоздаём событие
 */
$events = GetModuleEvents('linemedia.auto', 'OnBeforeOrdersEditPageBuild');
...
if($maxRole == "D") //учитываем только доступы в модуле Автоэксперт, модуль Интернет магазин не учитваем
{
    $APPLICATION->AuthForm(GetMessage("ACCESS_DENIED"));
}
...
{          
   ...
   // echo "chStatus"; var_dump($chStatusResultEvent);   
    ...
    && empty($dontsave))
{
   ...
   // if (CSaleOrder::IsLocked($ID, $lockedBY, $dateLock))
    ...
    if (strlen($STATUS_ID) > 0)
    {
		...
		if($curStatusPerms['PERM_STATUS']!='Y')
    ...
    while ($arOrderProps = $dbOrderProps->Fetch())
    {
        ...
        if (
            ($arOrderProps["IS_LOCATION4TAX"]=="Y")
            && IntVal($curVal) <= 0
            ||
            ($arOrderProps["IS_LOCATION"]=="Y" && $arOrderProps["REQUIED"]=="Y")
            && IntVal($curVal) <= 0
            ||
    ...
    /*
     * Проверка необходимых комментариев
     */
    if(COption::GetOptionString('linemedia.auto', 'LM_AUTO_MAIN_CANCEL_REQUIRE_COMMENT', 'N') == 'Y') {
        if($_REQUEST['CANCELED'] == 'Y' && empty($_REQUEST['REASON_CANCELED'])) {
            $errorMessage .= GetMessage("ERROR_CANCEL_REQUIRE_COMMENT")."<br>";
        }
    }
    if(is_array($LM_AUTO_MAIN_STATUS_REQUIRE_COMMENT) && count($LM_AUTO_MAIN_STATUS_REQUIRE_COMMENT) > 0) {
        if(in_array($_REQUEST['STATUS_ID'], $LM_AUTO_MAIN_STATUS_REQUIRE_COMMENT) && empty($_REQUEST['COMMENTS'])) {
            $statusName = '';
            $arStatus = LinemediaAutoProductStatus::getById($_REQUEST['STATUS_ID']);
            if($arStatus) $statusName = $arStatus['NAME'];
            $errorMessage .= str_replace('#STATUS#', $statusName, GetMessage("ERROR_STATUS_REQUIRE_COMMENT")) . "<br>";
        }
    }
...
/***** /DELIVERY *****/
    ...
    if (strlen($errorMessage) <= 0)
    {
        ...
        if (strlen($apply) <= 0)
            LocalRedirect("/bitrix/admin/".$arPageSettings['LIST_PAGE']."?lang=".LANG.GetFilterParams("filter_", false));
    }
    else
    {
...
elseif(!$lmCanEditOrder)
{
    CAdminMessage::ShowMessage(str_replace("#ID#", $ID, GetMessage("SOD_NO_PERMS2EDIT")).". ");    
}
   ...
   //     CSaleOrder::Lock($ID);
    ...
    $aMenu = array(
            array(
                ...
                "LINK" => "/bitrix/admin/".$arPageSettings['EDIT_PAGE']."?ID=".$ID."&dontsave=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_")
            )
        );
	...
	$aMenu[] = array(
		...
		"TITLE"=> GetMessage('LM_AUTO_SUPPLIERS_REQUEST'),
		"LINK" => "/bitrix/admin/linemedia.autosuppliers_out.php?order_ids=".$ID."&lang=".LANGUAGE_ID
	);

    if (/*$bUserCanViewOrder ||*/ $lmCanViewOrder && $lmCanViewStatus)
    {
        $aMenu[] = array(
    ...
    if (($saleModulePermissions == "W" || $str_PAYED != "Y") && (/*$bUserCanDeleteOrder ||*/ $lmCanEditOrder))
    {
        $aMenu[] = array(
                "TEXT" => GetMessage("SOEN_CONFIRM_DEL"),
                "LINK" => "javascript:if(confirm('".GetMessage("SOEN_CONFIRM_DEL_MESSAGE")."')) window.location='".$arPageSettings['LIST_PAGE']."?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get().urlencode(GetFilterParams("filter_"))."'",
                "WARNING" => "Y"
            );
    }
    ...
    else
    {
        ...
        ?>
					...
					<select name="STATUS_ID">
					   ...
					   if($curStatusPerms["PERM_STATUS_FROM"] == "Y" || $USER->IsAdmin())
                        ...
                        while($arStatusListTmp = $dbStatusListTmp->GetNext()) 
                         {
                            $select = "";
                            if ($arStatusListTmp["ID"] != $str_STATUS_ID) {          
            ...
            <tr>
                ...
                <td width="60%">
                    ...
                    <?=LinemediaAutoPrice::userAdminFormatCurrency($str_PRICE, $str_CURRENCY, $str_DATE_INSERT)?>
                </td>
            </tr>
            <tr>
                <td width="40%">
            ...
            <tr>
                ...
                <td width="60%">
                    <?=LinemediaAutoPrice::userAdminFormatCurrency($str_DISCOUNT_VALUE, $str_CURRENCY, $str_DATE_INSERT)?>
                </td>
            </tr>
            <tr>
                <td width="40%">
                ...
                <td width="60%">
				<?//println($curStatusPerms);?>
                    <input type="checkbox"<?if (/*!$bUserCanCancelOrder*/ !$lmCanEditOrder || !$curStatusPerms["PERM_CANCEL"] && !$USER->IsAdmin()) echo " disabled";?> name="CANCELED" value="Y"<?if ($str_CANCELED == "Y") echo " checked";?>>
                </td>
            </tr>
            <tr>
                ...
                <td width="60%" valign="top">
                    <textarea name="REASON_CANCELED"<?if (!$lmCanEditOrder || !$curStatusPerms["PERM_CANCEL"] && !$USER->IsAdmin()) echo " disabled";?> rows="2" cols="40"><?= $str_REASON_CANCELED ?></textarea>
                </td>
            </tr>
            <tr>
                <td colspan="2">
            ...
            while ($arProperties = $dbProperties->Fetch())
            {
                ...
                <tr>
                    <td align="right" valign="top">
                        <?
                        if ($arProperties["REQUIED"]=="Y" || /*$arProperties["IS_EMAIL"]=="Y" || */$arProperties["IS_PROFILE_NAME"]=="Y" || $arProperties["IS_LOCATION4TAX"]=="Y" ||
                            $arProperties["IS_PAYER"]=="Y")
                        {
                    ...
                    <td align="left">
                        ...
                        elseif ($arProperties["TYPE"] == "LOCATION")
                        {
                            ...
                            $dbVariants = CSaleLocation::GetList(
                                    ...
                                    false,
                                    false,
                                    array("ID", "COUNTRY_NAME", "REGION_NAME","CITY_NAME", "SORT")
                                );
                            while ($arVariants = $dbVariants->Fetch())
                            {
                                ...
                                if (IntVal($arVariants["ID"]) == IntVal($curVal) || !isset($curVal) && IntVal($arVariants["ID"]) == IntVal($arProperties["DEFAULT_VALUE"]))
                                    echo " selected";
                                echo '>'.htmlspecialcharsex($arVariants["COUNTRY_NAME"].((strlen($arVariants["REGION_NAME"]) > 0) ? " - " : "").$arVariants["REGION_NAME"].((strlen($arVariants["CITY_NAME"]) >
                                            0) ? " - " : "").$arVariants["CITY_NAME"]).'</option>';
                            }
        ...
        ?>
            ...
            <tr>
                ...
                <td width="60%">
                    <input type="checkbox"<?if (/*!$bUserCanPayOrder*/ !$lmCanEditOrder || !$curStatusPerms["PERM_PAYMENT"] && !$USER->IsAdmin()) echo " disabled";?> name="PAYED" OnClick="PayedClicked()" value="Y"<?if ($str_PAYED == "Y") echo " checked";?>>
                </td>
            </tr>
            <tr>
                <td width="40%">
                ...
                <td width="60%">
                    <input type="text" name="PAY_VOUCHER_NUM"<?if (/*!$bUserCanPayOrder*/ !$lmCanEditOrder || !$curStatusPerms["PERM_PAYMENT"] && !$USER->IsAdmin()) echo " disabled";?> value="<?= $str_PAY_VOUCHER_NUM ?>" size="20" maxlength="20">
                </td>
            </tr>
            <tr>
                <td width="40%">
                ...
                <td width="60%">
                    <?= CalendarDate("PAY_VOUCHER_DATE", $str_PAY_VOUCHER_DATE, "change_pay_form", "20", "class=\"typeinput\"".((/*!$bUserCanPayOrder*/ !$lmCanEditOrder || !$curStatusPerms["PERM_PAYMENT"] && !$USER->IsAdmin()) ? " disabled" : "")); ?>
                </td>
            </tr>
            <tr>
                <td colspan="2">
            ...
            <tr>
                ...
                <td width="60%">
                    <input type="checkbox" name="ALLOW_DELIVERY"<?if (/*!$bUserCanDeliverOrder*/ !$lmCanEditOrder || !$curStatusPerms["PERM_DELIVERY"] && !$USER->IsAdmin()) echo " disabled";?> value="Y"<?if ($str_ALLOW_DELIVERY == "Y") echo " checked";?>>
                </td>
            </tr>
        <tr>
                <td width="40%">
...
?>
        ...
        ?>
            ...
            else
            {
                ...
                while ($arBasket = $dbBasket->GetNext()) {
                    ...
                    while ($arBasketProps = $dbBasketProps->GetNext()) {
                        ...
                        //if($arBasketProps["CODE"] == "supplier_id")
                        //{
                        // $arBasket["PROPS"][$arBasketProps["CODE"]] = $arBasketProps;
                        //}
                        //println($arBasketProps) ;
                        
                        //$arBasket["PROPS"]['supplier_id'] = $arBasketProps;
                    }
           ...
           // println($arBasketItem);
            ...
            foreach ($arBasketItem as $val) {
                ...
                $supplierId = false;
                // если есть ключи
                if(array_key_exists('supplier_id', $val["PROPS"])) {
                    $supplierId = $val["PROPS"]["supplier_id"]["VALUE"];

                } else { // если нет
                    foreach($val["PROPS"] as $prop) {
                        if($prop['CODE'] == 'supplier_id') $supplierId = $prop['VALUE'];
                    }
                }
                 ...
                 * доступ + если админ
                 */
                if(LinemediaAutoOrder::isProductSupplierAvailable($supplierId) == "Y" || $USER->IsAdmin())
                {
                ?>
                <tr id="BASKET_TABLE_ROW_<?= $val["ID"] ?>">
                    ...
                    <td><div id="DIV_PROPS_<?= $val["ID"] ?>">
                        ...
                        foreach ($val["PROPS"] as $vv) {
                            if (strlen($vv["VALUE"]) > 0) {
                                if($vv["CODE"] == 'supplier_title') {
                                    echo $vv["NAME"].": ".$arListSuppliers[$supplierId]['NAME']."<br />";
                                } else if(CModule::IncludeModule('linemedia.autobranches') && $vv["CODE"] == 'retail_chain') {
                                    echo $vv["NAME"].": ".LinemediaAutoBranchRetailChainHelper::getHtmlChain(json_decode($vv["~VALUE"], true), $val["CURRENCY"])."<br />";
                                } else {
                    ...
                    <td><div id="DIV_PRICE_<?=$val["ID"]?>">
                            <?=LinemediaAutoPrice::userAdminFormatCurrency($val["PRICE"], $val["CURRENCY"], $val["DATE_INSERT"])?>
                        </div></td>
                    <td>
                        <? if ($val['CAN_ADMIN_EDIT'] && $lmCanEditOrder && $curStatusPerms["PERM_UPDATE"]) { ?>
                            <a href="javascript: ShowProductEdit(<?= $val["ID"] ?>);"><?= GetMessage("SOE_JS_EDIT") ?></a>
                        <? } ?>
                        <br/>
                        <? if ($val['CAN_ADMIN_DELETE'] && $lmCanEditOrder && $curStatusPerms["PERM_DELETE"] == "Y") { ?>
                            <a href="javascript: DeleteProduct(<?= $val["ID"] ?>);"><?= GetMessage("SOE_JS_DEL") ?></a>
                        <? } ?>
                        <? if (!$val['CAN_ADMIN_EDIT'] && !$val['CAN_ADMIN_DELETE'] && !$lmCanEditOrder && !$curStatusPerms["PERM_DELETE"] && !$curStatusPerms["PERM_UPDATE"]) { ?>
        ...
        /**
         * Добавление товара (поиск). (СЮДА НАДО ПОВЕСИТЬ ОБРАБОТКУ ДОБАВЛЕНИЯ)
         */
        function AddProducts(index)
        {
            var quantity = 1;
            
            window.open('/bitrix/admin/linemedia.auto_sale_search_new.php?lang=<?= LANGUAGE_ID ?>&LID=<?= CUtil::JSEscape($LID) ?>&func_name=FillProductFields', '', 'scrollbars=yes,resizable=yes,width=840,height=550,top='+parseInt((screen.height - 500)/2-14)+',left='+parseInt((screen.width - 840)/2-5));
        }
        ...
        <br>
        <input<?= ($lmCanEditOrder && $curStatusPerms["PERM_UPDATE"] || $USER->IsAdmin()) ? "" : " disabled" ?> type="submit" name="save" value="<?= GetMessage("SOE_BUT_SAVE") ?>" title="<?= GetMessage("SOE_BUT_SAVE_ALT") ?>">
        <input<?= ((/*$bUserCanEditOrder ||*/ $lmCanEditOrder) && $curStatusPerms["PERM_UPDATE"] || $USER->IsAdmin()) ? "" : " disabled" ?> type="submit" name="apply" value="<?= GetMessage("SOE_BUT_APPLY") ?>" title="<?= GetMessage("SOE_BUT_APPLY_ALT") ?>">
        <input<?= ((/*$bUserCanEditOrder ||*/ $lmCanEditOrder) && $curStatusPerms["PERM_UPDATE"] || $USER->IsAdmin()) ? "" : " disabled" ?> type="submit" name="dontsave" value="<?= GetMessage("SOE_BUT_CANCEL") ?>" title="<?= GetMessage("SOE_BUT_CANCEL_ALT") ?>">
