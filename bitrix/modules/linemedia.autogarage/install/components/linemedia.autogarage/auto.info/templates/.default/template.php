...
<? if (is_array($arResult['BRANDS']) && count($arResult['BRANDS']) > 0) { ?>
    <tr id="tr_brand">
        <th class="field-name" valign="top"><?= GetMessage('MARK') ?>:</th>
        <td id="td_brand">
            ...
            <? if ($arParams['BRAND_ID'] > 0) { ?>
                ...
                <?  // Popup с информацией
                    ...
                    $arDialogParams = array(
                      ...
                      'resizable'   => false,
                      'draggable'   => true,
                      'content_url' => $component->getPath().'/ajax_get_info.php?a=getBrand',
                   );
                            ...
                            <td>
                                <?  // Popup с информацией
                                    ...
                                    $arDialogParams = array(
                                      ...
                                      'resizable'   => false,
                                      'draggable'   => true,
                                      'content_url' => $component->getPath().'/ajax_get_info.php?a=getModel&BrandID='.$aBrand['manuId'],
                                   );
...
<tr id="tr_model"<? if ($arParams['MODEL_ID'] == 0 && $arParams['BRAND_ID'] == 0) { ?> class="auto-info-hide-tr"<? } ?>>
    <th class="field-name" valign="top"><?= GetMessage('MODEL') ?>:</th>
    <td id="td_model">
        <div class="dialog-content" style="display: none;"></div>
        <? if ($arParams['MODEL_ID'] > 0 && $arParams['BRAND_ID'] > 0) { ?>
            ...
            <?  // Popup с информацией
                ...
                $arDialogParams = array(
                  ...
                  'resizable'   => false,
                  'draggable'   => true,
                  'content_url' => '/bitrix/components/linemedia.autogarage/auto.info/ajax_get_info.php?a=getModel&BrandID='.$arParams['BRAND_ID'],
               );
...
<tr id="tr_modification"<?if($arParams['MODIFICATION_ID'] == 0 && $arParams['MODEL_ID'] == 0){?> class="auto-info-hide-tr"<?}?>>
    <th class="field-name" valign="top"><?= GetMessage('MODIFICATION') ?>:</th>
    <td id="td_modification">
        ...
        <?  // Popup с информацией
            ...
            $arDialogParams = array(
              ...
              'resizable'   => false,
              'draggable'   => true,
              'content_url' => '/bitrix/components/linemedia.autogarage/auto.info/ajax_get_info.php?a=GetModification&BrandID='.$arParams['BRAND_ID'].'&ModelID='.$arParams['MODEL_ID'],
           );
