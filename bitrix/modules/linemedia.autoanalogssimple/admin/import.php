<?php
/**
 * Административный файл для импорта кроссов
*/
 ...
 * Параметры групп
 */
if(isset($_REQUEST['ANALOG_GROUP_ORIGINAL'])) {
    $ANALOG_GROUP_ORIGINAL = $_REQUEST['ANALOG_GROUP_ORIGINAL'];
}
if(isset($_REQUEST['ANALOG_GROUP_ANALOG'])) {
    $ANALOG_GROUP_ANALOG = $_REQUEST['ANALOG_GROUP_ANALOG'];
}
 ...
 */
$first_names_r = 'N';
if(isset($_REQUEST['first_names_r'])) {
    $first_names_r = $_REQUEST['first_names_r'];
}
...
$arLMCatalogAvailProdFields = array(
    array("value"=>"group_original", "field"=>"group_original", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_ANALOG_GROUP")),
    array("value"=>"group_analog", "field"=>"group_analog", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_ANALOG_GROUP_REVERSE")),
	array("value"=>"article_original", "field"=>"article_original", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_ARTICLE_ORIGINAL")),
	array("value"=>"brand_original", "field"=>"brand_original", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_BRAND_ORIGINAL")),
	array("value"=>"article_analog", "field"=>"article_analog", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_ARTICLE_ANALOG")),
	array("value"=>"brand_analog", "field"=>"brand_analog", "important"=>"Y", "name"=>GetMessage("LM_AUTO_AS_BRAND_ANALOG")),
);

$defLMCatalogAvailProdFields = "article_original,brand_original,article_analog,brand_analog";

/////////////////////////////////////////////////////////////////////
/**
* Получить номер значения поля по коду
...
if (($REQUEST_METHOD == "POST" || $CUR_FILE_POS > 0) && $STEP > 1 && check_bitrix_sessid()) {
	...
	if ($STEP > 3) {
		...
		if (strlen($strError) <= 0) {
			...
			$strAvailProdFields = $defLMCatalogAvailProdFields;
			$arAvailProdFields = explode(",", $strAvailProdFields);
			$arAvailProdFields_names = array();
			for ($i = 0; $i < count($arAvailProdFields); $i++) {
				for ($j = 0; $j < count($arLMCatalogAvailProdFields); $j++) {
					if ($arLMCatalogAvailProdFields[$j]["value"]==$arAvailProdFields[$i]) {
						$arAvailProdFields_names[$arAvailProdFields[$i]] = array(
							"field" => $arLMCatalogAvailProdFields[$j]["field"],
							"important" => $arLMCatalogAvailProdFields[$j]["important"]
							);
						break;
					}
				}
...
/**
* Парсинг CSV файла
* @param object $csvFile Объект csv файла
*/
			...
			while ($arRes = FetchAssoc($csvFile)) {
                ...
                */
                ...
                try {
                    $analog->add(array(
                        'import_id'         => $import_id,
                        'group_original'    => $ANALOG_GROUP_ORIGINAL,
                        'group_analog'     => $ANALOG_GROUP_ANALOG,
				...
				if (intval($max_execution_time)>0 && (time()-START_EXEC_TIME)>intval($max_execution_time)) {
...
if (!$bAllLinesLoaded) {
	...
	$strParams = bitrix_sessid_get()."&CUR_FILE_POS=".$curFilePos."&CUR_LOAD_SESS_ID=".urlencode($CUR_LOAD_SESS_ID)."&STEP=4&URL_DATA_FILE=".urlencode($DATA_FILE_NAME)."&IBLOCK_ID=".$IBLOCK_ID."&fields_type="
		.urlencode($fields_type)."&max_execution_time=".IntVal($max_execution_time)."&ANALOG_GROUP_ORIGINAL=".intval($ANALOG_GROUP_ORIGINAL)."&ANALOG_GROUP_ANALOG=".intval($ANALOG_GROUP_ANALOG) ;

	for($i=0; $i < 10; $i++) {
		if (isset($_REQUEST["field_$i"])) {
			$strParams .= "&field_$i=". $_REQUEST["field_$i"];
		}
	}
	//.
		//"&field_0=article_original&field_1=brand_original&filed_2=article_analog&filed_3=brand_analog";
	if ($fields_type=="R")
...
if ($STEP == 1) {
	?>
	<tr>
        <td><?= GetMessage("IBLOCK_ADM_IMP_ANALOG_GROUP") ?>:</td>
        <td>
            <select name="ANALOG_GROUP_ORIGINAL">
                <? foreach ($arAnalogGroups as $id => $title) { ?>
                    <option value="<?= $id ?>" <?= ($ANALOG_GROUP_ORIGINAL == $id) ? ('selected') : ('') ?>>
                        <?= $title ?>
                    </option>
                <? } ?>
            </select>
    ...
    <tr>
        <td><?= GetMessage("IBLOCK_ADM_IMP_ANALOG_GROUP_REVERSE") ?>:</td>
        <td>
            <select name="ANALOG_GROUP_ANALOG">
                <? foreach ($arAnalogGroups as $id => $title) { ?>
                    <option value="<?= $id ?>" <?= ($ANALOG_GROUP_ANALOG == $id) ? ('selected') : ('') ?>>
                        <?= $title ?>
                    </option>
                <? } ?>
            </select>
...
if ($STEP == 3)
{
	...
	$strVal = $defLMCatalogAvailProdFields;
	$arVal = explode(",", $strVal);
	$arLMCatalogAvailProdFields_tmp = $arLMCatalogAvailProdFields;
	for ($i = 0; $i < count($arVal); $i++)
	{
		for ($j = 0; $j < count($arLMCatalogAvailProdFields_tmp); $j++)
		{
			if ($arVal[$i]==$arLMCatalogAvailProdFields_tmp[$j]["value"])
			{
				$arAvailFields[] = array("value"=>$arLMCatalogAvailProdFields_tmp[$j]["value"], "name"=>$arLMCatalogAvailProdFields_tmp[$j]["name"]);
				break;
			}
		}
	}
...
<?if ($STEP < 4):?>
	...
	<? if ($STEP > 1) { ?>
		...
		<input type="hidden" name="ANALOG_GROUP_ORIGINAL" value="<?= htmlspecialchars($ANALOG_GROUP_ORIGINAL) ?>" />
        <input type="hidden" name="ANALOG_GROUP_ANALOG" value="<?= htmlspecialchars($ANALOG_GROUP_ANALOG) ?>" />
