...
}elseif(isset($arVariables['ARTICLE_ID']))
{
	...
	$article['search_url'] = str_replace(
        array('#ARTICLE#', '#BRAND_TITLE#'),
        array($article['Article'], $article['Brand']),
        $arParams['SEARCH_URL']
    );
...
} elseif(isset($arVariables['GROUP_SECTION']))
{
    ...
    foreach ($articles as $key => $detail) {
        ...
        */
        ...
        $articles[$key]['search_url'] = str_replace(
	        array('#ARTICLE#', '#BRAND_TITLE#'),
	        array($detail['Article'], $brand_title),
	        $arParams['SEARCH_URL']
	    );
