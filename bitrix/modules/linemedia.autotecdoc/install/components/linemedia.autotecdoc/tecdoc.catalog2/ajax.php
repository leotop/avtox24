...
if (!CModule::IncludeModule('linemedia.auto') || !CModule::IncludeModule('linemedia.autotecdoc')) {
    die('no module');
}
 ...
 * АПИ модификаци
 */
$modifications = new \LinemediaAutoTecDocApiModifications;
...
if ($action == 'save_all') {
    ...
    if (is_array(unserialize($_REQUEST['literal_article']))) {
        $all_ids = array_merge($all_ids, unserialize($_REQUEST['literal_article']));
    }
...
if ($action == 'save') {
     ...
     */
    if ($mod_id != '') {
        $modifications->updateModificationByIdNew($type, $source_id, $set_id, $out);
        closeJsPopup();
        exit();
    }
...
if ($action == 'edit_window') {
    ...
    if ($source_id == '') {
         ...
         */
        $part = $modifications->getItemById($type, $source_id, $parent_id, false);
    }
    ?>
    <form action="/bitrix/components/linemedia.autotecdoc/tecdoc.catalog2/ajax.php" method="POST" id="lm-auto-tecdoc-popup-frm" enctype="multipart/form-data">
        ...
        <?= recursivePrintField($data_example, $part, !empty($source_id)); ?>

        <div class="lm-auto-popup-edit">
            <label>
                <div><?= GetMessage('LM_AUTO_API_FIELD_IMAGE') ?><span></span></div>

                <input type="hidden" name="out[image_path]" value="<?= htmlspecialchars($part['image']) ?>" />
                <input type="hidden" name="out[image_id]" value="<?= htmlspecialchars($part['image_id']) ?>" />

                <div class="lm-auto-image-edit">
                    <?= (!empty($part['image'])) ? (CFile::ShowImage($part['image_id'], 50, 50)) : (''); ?>
                </div>
                <?= CFile::InputFile('image', 50, $part['image_id']) ?>
            </label>
        </div>
        <div class="lm-auto-popup-edit">
            <label>
                <div><?= GetMessage('LM_AUTO_API_FIELD_HIDDEN') ?></div>
                <input type="checkbox" name="out[hidden]" value="Y" <?= $part['hidden'] == 'Y' ? 'checked' : '' ?> />
            </label>
        </div>
        <div class="lm-auto-popup-edit">
            <label>
                <div><?= GetMessage('LM_AUTO_API_FIELD_SORT') ?></div>
                <input type="text" name="out[sort]" value="<?= $part['sort'] ? $part['sort'] : 500 ?>" />
            </label>
        </div>
    <?if ($type =='group') {?>
        <div class="lm-auto-popup-edit">
            <label>
                <input type="checkbox" name="" <?if($parent_id=='*')echo ' checked="checked" ';?> value="" onclick="var ___lm_v = $(this).closest('form').find('input[name=mod_id]');if(___lm_v.val()=='*'){___lm_v.val('<?=$parent_id?>');}else{___lm_v.val('*');}">
    ...
    <?}?>
        ...
        <?  // Добавление пользовательских полей.
            ...
            while ($arEvent = $events->Fetch()) {
                $html .= ExecuteModuleEventEx($arEvent, array(&$part));
            }
            echo $html;
        ?>
        <? if (is_array($part['user_fields'])) {
            foreach ($part['user_fields'] as $code => $value) { ?>
                <div class="lm-auto-popup-edit">
