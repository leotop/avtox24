...
<div id="tecdoc-shortcuts">
<?foreach($arResult['SHORTCUTS'] AS $shortcut){?>
	<a href="#" class="tecdoc-shortcut" data-group-ids="<?=join(',', $shortcut['groups'])?>"><img src="<?=$shortcut['image']?>" alt="<?=$shortcut['title']?>" title="<?=$shortcut['title']?>"></a>
<?}?>
</div>
...
if (!function_exists('lm_recPrintTree')) {
    function lm_recPrintTree($parent_id, &$arResult, $arParams, $folder)
    {
        ...
        foreach ($arResult['GROUPS'] as $i => $group) {
            ...
            $out .= '<li data-id="'.$group['assemblyGroupNodeId'].'">';
