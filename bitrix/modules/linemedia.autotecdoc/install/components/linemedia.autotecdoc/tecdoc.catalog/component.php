...
if (empty($arParams['SEARCH_URL'])) {
    $arParams['SEARCH_URL'] = '/auto/search/#ARTICLE#/';
}
...
{
    $sLM_AUTOPORTAL_TECDOC_ACCESS_LIST_IBLOCK_ID = COption::GetOptionInt('linemedia.autotecdoc', 'LM_AUTO_IBLOCK_TECDOC_ACCESS_LIST', false);
 ...
 * а если нет тогда стандартный ("#BRAND#/#MODEL#/...")
 */
 ...
 * Коды доступа
 */
$api_id  = COption::GetOptionString('linemedia.autotecdoc', 'LM_AUTO_TECDOC_API_ID');
$api_key = COption::GetOptionString('linemedia.autotecdoc', 'LM_AUTO_TECDOC_API_KEY');

$oAutoportal = new LinemediaAutoTecDocApiDriver($api_id, $api_key);
...
if (isset($MODEL) && trim($MODEL) != '') {
    ...
    if(!$arResult['model_id'] || !preg_match('/^\d+$/', $MODEL)) CHTTP::SetStatus("404 Not Found");
}       

if (isset($CAR_ID) && trim($CAR_ID) != '') {
    $arResult['car_id'] = intval($CAR_ID);
    if(!$arResult['car_id'] || !preg_match('/^\d+$/', $CAR_ID)) CHTTP::SetStatus("404 Not Found");
}   
    ...
    if (empty($arResult['brand_id'])) {
        ...
        try {
            ...
            foreach ($arResult['BRANDS'] as &$val) {
                $logo = '/upload/linemedia.autotecdoc/images/brands/'.str_replace(array(' '), array('_'), $val['manuName']).'.png';
         ...
         */
        try {
            $aDetRes = $oAutoportal->query('getDetails',
                                  $data = array('type_id'=>$arResult['car_id'],
                                        'group_id'=>$arResult['group_id']),
                                  $in = 'serialized');
            ...
            foreach ($arResult['DETAILS'] as $key => $detail) {
                ...
                $findurl = LinemediaAutoTecDocUrlHelper::getPartUrl(array(
                        ...
                        'extra' => array_merge($detail, array('tecdoc_brand_id' => $detail['brandNo'])),
                    ),
                    $arParams['SEARCH_URL']
                );
