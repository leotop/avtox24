        ...
        <? if ($bIsAdmin) { ?>
            $('.f_access').change(function() {
                ...
                if (CheckItem.checked) {
                    // показываем
                    $.get('<?= $component->getPath() ?>/ajax.php?a=enable&s=' + encodeURIComponent($(CheckItem).attr('name')) + '&id=' + encodeURIComponent($(CheckItem).val()), function(data){$(CheckItem).removeAttr('disabled');});
                } else {
                    // скрываем
                    $.get('<?= $component->getPath() ?>/ajax.php?a=disable&s=' + encodeURIComponent($(CheckItem).attr('name')) + '&id=' + encodeURIComponent($(CheckItem).val()), function(data){$(CheckItem).removeAttr('disabled');});
                }
            });
            ...
            $('#ApplyForAllAuto').click (function () {
                ...
                $.post('<?= $component->getPath() ?>/ajax.php?s=SET_GROUP_FOR_ALL_AUTO', { id: aDisableItems},
                   function(data) {
...
<?elseif(isset($arResult['GROUPS'])):?>
    ...
    $aDisableItem = Array();
    ?>
    <ul id="navigation-tecdoc" class="tecdoc">
        ...
        <tbody>
            <? foreach($arResult['DETAILS'] as $value) { ?>
                <tr>
                    ...
                    <td width="110">
                        <? $folder = COption::GetOptionString('linemedia.autotecdoc', 'LM_AUTO_TECDOC_DEMO_FOLDER'); ?>
                        <a href="/tecdoc-part-detail/<?= $value['articleId'] ?>/<?= $value['articleLinkId'] ?>/">
                            <?= GetMessage('INFO') ?>
                        </a>
                    </td>
                    <td width="120">
